...
from BrainSimulator.models.vqvae import VQVAE
...
def evaluate_test_loss(model, test_loader, device):
    ...
    return {
        ...
        'test_vq_loss': total_vq_loss / n_batches
    }
...
def plot_losses(train_losses, recon_losses, vq_losses, test_losses, exp_dir, train_loader):
    ...
    if len(train_losses) > 0:
        ...
        plt.plot(iterations, vq_losses, label='Train VQ Loss', linewidth=2, alpha=0.7)
        ...
        if len(test_losses) > 1:  # Need at least 2 points for interpolation
            ...
            f_vq = interp1d(test_iterations, [x['test_vq_loss'] for x in test_losses], kind='linear')
            ...
            plt.plot(x_smooth, f_vq(x_smooth), '--', label='Test VQ Loss', linewidth=2, alpha=0.7)
        
        plt.xlabel('Iteration')
        plt.ylabel('Loss')
        plt.title('VQ-VAE Training and Test Losses')
        plt.legend()
        plt.grid(True)
...
def train_vqvae(train_loader, test_loader, model, optimizer, device, epoch, train_losses, 
               recon_losses, vq_losses, test_losses, exp_dir, test_dataset, recording_dirs):
    model.train()
    ...
    for batch_idx, data in enumerate(tqdm(train_loader, desc=f"Epoch {epoch}")):
        ...
        if batch_idx % 100 == 0:
            ...
            plot_losses(train_losses, recon_losses, vq_losses, test_losses, exp_dir, train_loader)
    ...
    test_loss_dict_for_csv = {
        ...
        'vq_loss': [x['test_vq_loss'] for x in test_losses]
    }
...
def main():
    try:
        ...
        model_params = {
            'image_height': height,
            'image_width': width,
            'num_embeddings': 65536,  # Increased codebook size (8x again)
            'embedding_dim': 1024,    # Increased embedding dimension (2x again)
            'commitment_cost': 0.25   # Beta in the VQ-VAE paper
        }
        
        training_params = {
            'batch_size': 16,         # Further reduced batch size for larger model
            'epochs': 5,
            'learning_rate': 2e-4,    # Further reduced learning rate for stability
        ...
        model = VQVAE(
            ...
            model_params['num_embeddings'],
            model_params['embedding_dim'],
            model_params['commitment_cost']
        )
        ...
        with open(os.path.join(exp_dir, "model_architecture.txt"), "w") as f:
            ...
            f.write(f"Codebook size: {model_params['num_embeddings']}\n")
            f.write(f"Embedding dimension: {model_params['embedding_dim']}\n")
        ...
        for epoch in range(1, training_params['epochs'] + 1):
            train_vqvae(train_loader, test_loader, model, optimizer, device, epoch, 
                      train_losses, recon_losses, vq_losses, test_losses, exp_dir, 
                      test_dataset, recording_dirs)
            
            # Save checkpoint every 4 epochs
            if epoch % 4 == 0:
                checkpoint_path = os.path.join(exp_dir, "checkpoints", f"vqvae_epoch_{epoch}.pt")
                torch.save({
                    'epoch': epoch,
        ...
        # Save final model
        final_model_path = os.path.join(exp_dir, "checkpoints", "vqvae_final.pt")
