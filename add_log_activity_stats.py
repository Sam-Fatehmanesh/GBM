...
def process_file(path: str, eps: float, overwrite: bool) -> Optional[str]:
    try:
        with h5py.File(path, "r+") as f:
            if (
                ("log_activity_mean" in f)
                and ("log_activity_std" in f)
                and (not overwrite)
            ):
                return None
            ...
            for t0 in range(0, T, chunk_T):
                ...
                sum_sq_logs += np.square(logs, dtype=np.float64).sum(
            ...
            f.create_dataset(
                "log_activity_mean", data=mean, compression="gzip", compression_opts=1
            )
            f.create_dataset(
                "log_activity_std", data=std, compression="gzip", compression_opts=1
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Add per-neuron log(activity) mean/std to GBM H5 outputs"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
        required=True,
        help="Directory containing subject_*.h5 files",
    )
    parser.add_argument(
        "--eps", type=float, default=1e-7, help="Epsilon to avoid log(0)"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite log stats if already present",
    )
    parser.add_argument(
        "--subjects-glob",
        type=str,
        default="subject_*.h5",
        help="Glob to select subject files",
    )
