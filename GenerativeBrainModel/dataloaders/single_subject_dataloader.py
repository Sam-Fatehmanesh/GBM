...
def create_single_subject_dataloaders(config: Dict) -> Tuple[DataLoader, DataLoader, None, None, torch.Tensor]:
    ...
    if include_files:
        subject_name = include_files[0] if isinstance(include_files, list) else include_files
    else:
        # fallback to subject_14.h5 if not provided
        subject_name = 'subject_14.h5'
    subject_path = (data_dir / subject_name).resolve()
    if not subject_path.exists():
        raise FileNotFoundError(f"Single-subject dataloader: file not found: {subject_path}")
    ...
    dl_kwargs = {
        ...
        'persistent_workers': train_cfg.get('persistent_workers', False) if num_workers > 0 else False,
    ...
    train_loader = DataLoader(train_dataset, shuffle=True, sampler=None, collate_fn=_collate_pad, **dl_kwargs)
    val_loader = DataLoader(val_dataset, shuffle=False, sampler=None, collate_fn=_collate_pad, **dl_kwargs)
