...
class BehaviorDataset(Dataset):
    ...
    def __init__(
        ...
        self.cached_behavior: Optional[Dict[str, np.ndarray]] = (
            {} if use_cache else None
        )
    ...
    def _build_index(self) -> None:
        ...
        for file_path in self.data_files:
            with h5py.File(file_path, "r") as f:
                ...
                # Determine bounds
                start_point = (
                    self.start_timepoint if self.start_timepoint is not None else 0
                )
                end_point = (
                    self.end_timepoint if self.end_timepoint is not None else T_total
                )
                ...
                if self.use_cache:
                    ...
                    if beh.ndim == 1:
                        ...
                        print(
                            f"Skipping {Path(file_path).name}: unexpected behavior_full ndim={beh.ndim}"
                        )
                        continue
                    ...
                    if B != self.pad_behavior_to:
                        if B < self.pad_behavior_to:
                            pad = np.zeros(
                                (self.pad_behavior_to - B, T_slice), dtype=np.float32
                            )
                ...
                for s in range(trimmed_start, trimmed_end - L + 1, max(1, self.stride)):
                    ...
                    self.sequences.append(
                        {"file_path": file_path, "window_start": s, "start_idx": curr}
                    )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_cache:
            ...
            if beh.ndim == 1:
                ...
                raise RuntimeError(
                    f"Unexpected behavior_full ndim={beh.ndim} in {file_path}"
                )
            # Sanitize raw behavior
            try:
                beh_bt = np.nan_to_num(beh_bt, nan=0.0, posinf=0.0, neginf=0.0)
...
def create_behavior_dataloaders(
    config: Dict,
) -> Tuple[
    ...
    if not test_files:
        print(
            "No valid test subjects provided. Creating a random 80/20 train/test split."
        )
    ...
    def collate_pad(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        spikes = torch.stack(
            [pad_spikes(it["spikes"], target_n) for it in batch], dim=0
        )  # (B, 1, Npad)
        positions = torch.stack(
            [pad_positions(it["positions"], target_n) for it in batch], dim=0
        )  # (B, Npad, 3)
        masks = torch.stack(
            [pad_mask(it["neuron_mask"], target_n) for it in batch], dim=0
        )  # (B, Npad)
    ...
    val_loader = DataLoader(
        val_dataset, shuffle=False, sampler=None, collate_fn=collate_pad, **dl_kwargs
    )
