...
class NeuralDataset(Dataset):
    ...
    def __init__(
        ...
        self.cached_positions: Optional[Dict[str, np.ndarray]] = (
            {} if use_cache else None
        )
        self.cached_stimulus: Optional[Dict[str, np.ndarray]] = (
            {} if use_cache else None
        )
        self.cached_log_mean: Optional[Dict[str, np.ndarray]] = (
            {} if use_cache else None
        )
        self.cached_log_std: Optional[Dict[str, np.ndarray]] = {} if use_cache else None
        self.cached_neuron_ids: Optional[Dict[str, np.ndarray]] = (
            {} if use_cache else None
        )
            ...
            with h5py.File(file_path, "r") as f:
                ...
                if self.split_role == "test":
                    ...
                    start_point = (
                        ...
                        else max(split_idx, self.start_timepoint)
                    )
                    end_point = (
                        T_total
                        if self.end_timepoint is None
                        else min(T_total, self.end_timepoint)
                    )
                else:
                    ...
                    split_idx = max(0, min(split_idx, T_total))
                    start_point = (
                        0 if self.start_timepoint is None else self.start_timepoint
                    )
                    end_point = (
                        split_idx
                        if self.end_timepoint is None
                        else min(split_idx, self.end_timepoint)
                    )
                end_point = min(end_point, T_total)

                if self.max_timepoints_per_subject is not None:
                    end_point = min(
                        end_point, start_point + self.max_timepoints_per_subject
                    )

                num_timepoints = end_point - start_point
                if num_timepoints < self.sequence_length:
                    print(
                        f"Skipping {Path(file_path).name}: not enough timepoints ({num_timepoints}) for sequence_length={self.sequence_length}"
                    )
                    continue
                ...
                if nz_start >= end_point or nz_last < nz_start:
                    print(
                        f"Skipping {Path(file_path).name}: all-zero in selected window [{start_point},{end_point})"
                    )
                    continue
                ...
                if trimmed_start > start_point or trimmed_end < end_point:
                    ...
                    print(
                        f"Trimmed {Path(file_path).name}: lead_zero={lead}, tail_zero={tail}, kept={trimmed_end - trimmed_start}"
                    )
                else:
                ...
                if self.use_cache:
                    if file_path not in self.cached_spikes:
                        self.cached_spikes[file_path] = spikes_ds[
                    ...
                    if "stimulus_full" in f:
                        ...
                        self.cached_stimulus[file_path] = stim_oh.astype(
                            np.float32
                        )  # (T, K)
                    else:
                        # default to zeros with one channel
                        self.cached_stimulus[file_path] = np.zeros(
                            (trimmed_end - trimmed_start, 1), dtype=np.float32
                        )
                    # Log activity stats per neuron (optional)
                    try:
                        if "log_activity_mean" in f:
                            self.cached_log_mean[file_path] = f["log_activity_mean"][
                                :
                            ].astype(np.float32)
                        if "log_activity_std" in f:
                            self.cached_log_std[file_path] = f["log_activity_std"][
                                :
                            ].astype(np.float32)
                    except Exception:
                        pass
                ...
                for start_idx in range(trimmed_start, max_start_idx + 1, self.stride):
                    self.sequences.append(
                        {"file_path": file_path, "start_idx": start_idx}
                    )
    ...
    def _pad_stimulus(self, stim_2d: np.ndarray) -> np.ndarray:
        ...
        out[:, : min(K, self.pad_stimuli_to)] = stim_2d[
            :, : min(K, self.pad_stimuli_to)
        ]
        return out
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_cache:
            ...
            offset = (
                self.cache_offset[file_path]
                if self.cache_offset is not None
                else (self.start_timepoint or 0)
            )
            ...
            log_mean = (
                self.cached_log_mean.get(file_path, None)
                if self.cached_log_mean is not None
                else None
            )
            log_std = (
                self.cached_log_std.get(file_path, None)
                if self.cached_log_std is not None
                else None
            )
            ...
            if "stimulus_full" in f:
                stimulus = f["stimulus_full"][start_idx:end_idx].astype(
                    np.float32
                )  # (L, K_file)
            else:
            ...
            try:
                log_mean = (
                    f["log_activity_mean"][:].astype(np.float32)
                    if "log_activity_mean" in f
                    else None
                )
                log_std = (
                    f["log_activity_std"][:].astype(np.float32)
                    if "log_activity_std" in f
                    else None
                )
            except Exception:
        ...
        # Convert to tensors
        spikes_tensor = torch.from_numpy(
            segment_padded.astype(np.float32)
        )  # (L, pad_N)
        positions_tensor = torch.from_numpy(
            positions_padded.astype(np.float32)
        )  # (pad_N, 3)
        ...
        return {
            ...
            "log_activity_mean": (
                torch.from_numpy(log_mean) if log_mean is not None else torch.empty(0)
            ),  # (N,)
            "log_activity_std": (
                torch.from_numpy(log_std) if log_std is not None else torch.empty(0)
            ),  # (N,)
            "file_path": file_path,
            "start_idx": start_idx,
        }
    ...
    def _resolve_spikes_dataset_name(self, f: h5py.File) -> str:
        ...
        if "zcalcium" in f:
            return "zcalcium"
        raise ValueError(
            "No compatible spikes dataset found (tried neuron_values, spike_probabilities, spike_rates_hz, processed_calcium, zcalcium)"
        )
...
def create_dataloaders(
    ...
    unique_ids_tensor = (
        torch.from_numpy(unique_ids_np.astype(np.int64))
        if unique_ids_np.size > 0
    ...
    def collate_pad(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        stimulus = torch.stack(
            [it["stimulus"] for it in batch], dim=0
        )  # (B, L, K) already padded across files
        neuron_ids = torch.stack(
            [pad_ids(it["neuron_ids"], max_n) for it in batch], dim=0
        )
        ...
        log_std = torch.stack(
            [
                pad_vec_or_zero(it["log_activity_std"], max_n, torch.float32)
                for it in batch
            ],
            dim=0,
        )
        ...
        return {
            ...
            "start_idx": torch.tensor(
                [it["start_idx"] for it in batch], dtype=torch.long
            ),
        }
