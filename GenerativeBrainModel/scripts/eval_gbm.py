...
def load_model_from_checkpoint(ckpt_path: Path, device: torch.device) -> GBM:
    ...
    def infer_config_from_state_dict(sdict: Dict[str, torch.Tensor]) -> Dict[str, int]:
        ...
        for key, tensor in sdict.items():
            if key.endswith("head.1.weight") or key.endswith(
                "neuron_scalar_decoder_head.1.weight"
            ):
                if tensor.ndim == 2:
    ...
    if (
        ...
        d_model = (
            inferred["d_model"]
            if not (isinstance(model_cfg, dict) and model_cfg.get("d_model"))
            else model_cfg["d_model"]
        )
        d_stimuli = (
            inferred["d_stimuli"]
            if not (isinstance(model_cfg, dict) and model_cfg.get("d_stimuli"))
            else model_cfg["d_stimuli"]
        )
        n_layers = (
            inferred["n_layers"]
            if not (
                isinstance(model_cfg, dict) and model_cfg.get("n_layers") is not None
            )
        ...
        n_heads = (
            inferred["n_heads"]
            if not (
                isinstance(model_cfg, dict) and model_cfg.get("n_heads") is not None
            )
    ...
    try:
        ...
        if total > 0 and loaded / total < 0.5:
            print(
                f"[eval_gbm] Warning: Loaded only {loaded}/{total} keys from checkpoint. Model config may be mismatched."
            )
            if len(unexpected) > 0:
...
def evaluate(
    ...
    *,
    ...
    spikes_are_zcalcium: bool = False,
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc="Eval"), 1):
        ...
        x_tgt = spikes[:, 1:, :]
        stim_in = stim[:, :-1, :]
        mu_raw, log_sigma_raw, eta_raw, log_delta_raw = model(
    ...
    avg_mae_rate = (
        (mae_rate_sums / max(1, mae_rate_counts))
        if mae_rate_counts > 0
        else float("nan")
    )
...
def make_videos(
    ...
    sequence_length: int,
    num_batches: int = 1,
    use_double_window_for_ar: bool = False,
    *,
    spikes_are_rates: bool = False,
    spikes_are_zcalcium: bool = False,
    sampling_rate_hz: float = 3.0,
    ...
    for i in range(num_batches):
        ...
        preds_next = sas_rate_median(
            mu_raw, log_sigma_raw, eta_raw, log_delta_raw
        ).float()
        truth_next = x_tgt.float()
        create_nextstep_video(
            truth_next, preds_next, positions, videos_dir / f"nextstep_batch{i + 1}.mp4"
        )
        ...
        if n_steps > 0:
            ...
            if future_real.shape[1] < n_steps:
                pad = torch.zeros(
                    (
                        future_real.shape[0],
                        n_steps - future_real.shape[1],
                        future_real.shape[2],
                    ),
                    device=future_real.device,
                    dtype=future_real.dtype,
                )
            ...
            create_autoregression_video(
                gen_only,
                positions,
                videos_dir / f"autoreg_batch{i + 1}.mp4",
                truth=truth_only,
            )
    ...
    try:
        ...
        with h5py.File(out_h5, "w") as f:
            if ns_truth_list:
                f.create_dataset(
                    "truth_next_step",
                    data=torch.cat(ns_truth_list, dim=0).to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "pred_next_step",
                    data=torch.cat(ns_pred_list, dim=0).to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                ...
                try:
                    ...
                    f.create_dataset(
                        "positions",
                        data=ns_pos_list[0].to(torch.float32).numpy(),
                        compression="gzip",
                        compression_opts=1,
                    )
                try:
                    f.create_dataset(
                        "neuron_mask",
                        data=torch.cat(ns_mask_list, dim=0).numpy(),
                        compression="gzip",
                        compression_opts=1,
                    )
                except Exception:
            ...
            if ar_truth_list:
                f.create_dataset(
                    "truth_future",
                    data=torch.cat(ar_truth_list, dim=0).to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "pred_future",
                    data=torch.cat(ar_pred_list, dim=0).to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "ar_context_len", data=np.array(ar_context_list, dtype=np.int32)
                )
                f.create_dataset(
                    "ar_n_steps", data=np.array(ar_nsteps_list, dtype=np.int32)
                )
    except Exception:
        pass
...
def evaluate_autoregression(
    ...
    *,
    ...
    """
    per_step_loss_sums: List[
        float
    ] = []  # BCE in prob mode; MSE in zcalcium; N/A in rates
    # PR AUC removed
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc="Eval AR"), 1):
        ...
        for b in range(B):
            ...
            for k in range(n_steps):
                pred_k = gen_only[
                    b : b + 1, k, :
                ].detach()  # Keep batch dim for consistency
                ...
                if "neuron_mask" in batch:
                    ...
                    valid_mae = (
                        (pred_rate - true_rate).abs() * sample_mask.float()
                    ).sum() / sample_mask.sum().clamp_min(1)
                else:
            ...
            sample_score = (
                np.mean(sample_mae_rates) if sample_mae_rates else float("inf")
            )
            ...
            if sample_score < best_sample_score:
                ...
                best_sample_data = {
                    "initial_context": init_x[b : b + 1, :, :]
                    .detach()
                    .cpu(),  # (1, context_len, N)
                    "true_future": tgt_only[b : b + 1, :, :]
                    .detach()
                    .cpu(),  # (1, n_steps, N)
                    "pred_future": gen_only[b : b + 1, :, :]
                    .detach()
                    .cpu(),  # (1, n_steps, N)
                    "positions": positions[b : b + 1, :, :].detach().cpu(),  # (1, N, 3)
                    "neuron_mask": mask[b : b + 1, :].detach().cpu()
                    if "neuron_mask" in batch
                    else torch.ones(1, init_x.shape[-1], dtype=torch.bool),  # (1, N)
                    "stimulus_context": init_stim[b : b + 1, :, :].detach().cpu()
                    if init_stim is not None
                    else None,  # (1, context_len, K)
                    "stimulus_future": future_stim[b : b + 1, :n_steps, :]
                    .detach()
                    .cpu()
                    if future_stim is not None and future_stim.shape[1] >= n_steps
                    else None,  # (1, n_steps, K)
                    "context_len": context_len,
                    "n_steps": n_steps,
        ...
        for k in range(n_steps):
            ...
            if spikes_are_zcalcium:
                loss_k = F.mse_loss(
                    preds_k.float(), targs_k.float(), reduction="mean"
                ).item()
    ...
    for k in range(len(per_step_counts)):
        if spikes_are_zcalcium:
            ...
            per_step_loss.append(per_step_loss_sums[k] / max(1, per_step_counts[k]))
    per_step_mae_rate = [
        per_step_mae_rate_sums[k] / max(1, per_step_counts[k])
        for k in range(len(per_step_counts))
    ]
    per_step_mae_z = [
        per_step_mae_z_sums[k] / max(1, per_step_counts[k])
        for k in range(len(per_step_counts))
    ]
    per_step_mae_rate_baseline = [
        per_step_mae_rate_baseline_sums[k] / max(1, per_step_counts[k])
        for k in range(len(per_step_counts))
    ]
    ...
    with open(out_csv, "w") as f:
        if spikes_are_zcalcium:
            ...
            for k in range(len(per_step_counts)):
                f.write(
                    f"{k + 1},{per_step_loss[k]:.6f},{per_step_mae_z[k]:.6f},{per_step_counts[k]}\n"
                )
        else:
            ...
            for k in range(len(per_step_counts)):
                ...
                f.write(
                    f"{k + 1},{per_step_loss[k]:.6f},{per_step_mae_rate[k]:.6f},{rp_str},{per_step_counts[k]}\n"
                )
    ...
    try:
        ...
        fig.tight_layout()
        fig.savefig(
            logs_dir / "autoregression_per_step.png", dpi=120, bbox_inches="tight"
        )
        plt.close(fig)
        ...
        fig2.tight_layout()
        fig2.savefig(
            logs_dir / "autoregression_relative_per_step.png",
            dpi=120,
            bbox_inches="tight",
        )
        plt.close(fig2)
...
@torch.no_grad()
def create_best_ar_sample_video(
    best_sample_data: Dict[str, Any], videos_dir: Path
) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            h5_path = (
                videos_dir / f"best_ar_sample_batch{batch_idx}_idx{sample_idx}_data.h5"
            )
            with h5py.File(h5_path, "w") as f:
                f.create_dataset(
                    "initial_context",
                    data=init_context.to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "true_future",
                    data=true_future.to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "pred_future",
                    data=pred_future.to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "positions",
                    data=positions.to(torch.float32).numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                f.create_dataset(
                    "neuron_mask",
                    data=neuron_mask.numpy(),
                    compression="gzip",
                    compression_opts=1,
                )
                ...
                if best_sample_data.get("stimulus_context") is not None:
                    f.create_dataset(
                        "stimulus_context",
                        data=best_sample_data["stimulus_context"]
                        .to(torch.float32)
                        .numpy(),
                        compression="gzip",
                        compression_opts=1,
                ...
                if best_sample_data.get("stimulus_future") is not None:
                    f.create_dataset(
                        "stimulus_future",
                        data=best_sample_data["stimulus_future"]
                        .to(torch.float32)
                        .numpy(),
                        compression="gzip",
                        compression_opts=1,
                ...
                f.attrs["description"] = (
                    f"Best performing AR sample from batch {batch_idx}, sample {sample_idx} with MAE rate score {sample_score:.6f}"
                )
    ...
    for batch in tqdm(loader, desc="Spike trace plots"):
        ...
        for b in range(B):
            ...
            for ax_idx, neuron_id in enumerate(chosen):
                ax = axes[ax_idx]
                truth_np = (
                    spikes[b, :, neuron_id].detach().to(torch.float32).cpu().numpy()
                )
                pred_np = (
                    gen_only[b, :, neuron_id].detach().to(torch.float32).cpu().numpy()
                )
                ax.plot(
                    t_full, truth_np, label="Truth", color="tab:green", linewidth=1.5
                )
                ax.plot(
                    t_pred, pred_np, label="AR Pred", color="tab:orange", linewidth=1.5
                )
...
def main():
    ...
    # Device
    device = torch.device(
        "cuda" if (cfg["eval"]["use_gpu"] and torch.cuda.is_available()) else "cpu"
    )
    if device.type == "cuda":
    ...
    if ckpt and Path(ckpt).exists():
        ...
        try:
            ...
            model.spikes_are_rates = False
        try:
            model.spikes_are_zcalcium = bool(
                cfg["data"].get("spikes_are_zcalcium", False)
            )
        except Exception:
            model.spikes_are_zcalcium = False
    else:
        if not load_from:
            raise ValueError(
                "Please specify model.checkpoint or eval.load_from (path to eval_data.h5)"
            )
    ...
    try:
        ...
        if (
            "sequence_length" not in cfg["eval"]
            or cfg["eval"]["sequence_length"] is None
        ):
            cfg["eval"]["sequence_length"] = tr_cfg.get(
        ...
        if cfg["data"].get("test_subjects") in (
            None,
            [],
            (),
        ):  # inherit test subjects if present
    ...
    if isinstance(test_subjects, list) and len(test_subjects) > 0:
        ...
        if missing:
            raise ValueError(
                f"Requested test subjects not found in {data_dir}: {missing}"
            )
    ...
    tracker = CombinedMetricsTracker(
        log_dir=dirs["logs"],
        ema_alpha=0.0,
        val_threshold=cfg["eval"]["threshold"],
        enable_plots=True,
    )
    if not offline_mode:
        metrics = evaluate(
    ...
    if not offline_mode:
        try:
            ...
            logger.info(
                f"Autoregression per-step metrics saved. First steps MAE_rate={ar_metrics['per_step_mae_rate'][:3]}"
            )
            ...
            if ar_metrics.get("best_sample_data") is not None:
                ...
                logger.info(
                    f"Best AR sample found: batch {best_data['batch_idx']}, sample {best_data['sample_idx']} with MAE rate score {best_score:.6f}"
                )
                ...
                try:
                    ...
                    if created:
                        logger.info(
                            f"Created video for best AR sample: {dirs['videos'] / video_filename}"
                        )
                    else:
                        logger.warning(
                            "Best AR sample video creation reported failure."
                        )
                except Exception as e:
    ...
    try:
        ...
        if offline_mode and load_from and Path(load_from).exists():
            with h5py.File(load_from, "r") as f:
                ...
                mask = (
                    torch.from_numpy(f["neuron_mask"][()]).bool()
                    if "neuron_mask" in f
                    else None
                )  # (Bsum, N)
                if mask is not None:
                    mask_rep = mask[:, None, :].expand(
                ...
                bce = (
                    F.binary_cross_entropy(p, t, reduction="none")
                ...
                for j in range(min(Tpos, bce.shape[1])):
                    ...
                    if j - 1 >= 0 and j - 1 < baseline_rate.shape[1]:
                        abs_err_bl = (
                            baseline_rate[:, j - 1, :] - t_rate[:, j, :]
                        ).abs()
                        step_mae_rate_baseline_sums[j] += (
                            abs_err_bl[sel].sum().double().cpu()
                        )
        else:
            with torch.no_grad():
                used = 0
                ...
                for batch in tqdm(ns_loader, desc="Next-step loss vs context"):
                    ...
                    baseline_rate = t_rate[
                        :, :-1, :
                    ]  # aligns with next-step positions j>=1
                    for j in range(min(Tpos, bce.shape[1])):
                        sel = mask_rep[:, j, :]
                        if not bool(cfg["data"].get("spikes_are_rates", False)):
                            step_loss_sums[j] += bce[:, j, :][sel].sum().double().cpu()
                        step_loss_counts[j] += sel.sum().double().cpu()
                        step_mae_rate_sums[j] += (
                            abs_err[:, j, :][sel].sum().double().cpu()
                        )
                        if j - 1 >= 0 and j - 1 < baseline_rate.shape[1]:
                            abs_err_bl = (
                                baseline_rate[:, j - 1, :] - t_rate[:, j, :]
        ...
        step_mae_rate = (
            (step_mae_rate_sums / step_loss_counts.clamp_min(1)).cpu().numpy()
        )
        step_mae_rate_baseline = (
            (step_mae_rate_baseline_sums / step_loss_counts.clamp_min(1)).cpu().numpy()
        )
        # Relative performance per position
        rel_perf = np.full_like(step_mae_rate, np.nan, dtype=np.float64)
        valid = (
            (step_mae_rate_baseline > 1e-12)
            & np.isfinite(step_mae_rate_baseline)
            & np.isfinite(step_mae_rate)
        )
        ...
        with open(dirs["logs"] / "nextstep_loss_vs_context.csv", "w") as fcsv:
            ...
            for c, loss, mae_r, rp, cnt in zip(
                ctx_sizes,
                step_loss,
                step_mae_rate,
                rel_perf,
                step_loss_counts.cpu().numpy(),
            ):
                rp_str = f"{float(rp):.6f}" if np.isfinite(rp) else "nan"
                fcsv.write(
                    f"{int(c)},{float(loss):.6f},{float(mae_r):.6f},{rp_str},{int(cnt)}\n"
                )
        # Plot scatter
        ...
        plt.tight_layout()
        plt.savefig(
            dirs["logs"] / "nextstep_loss_vs_context.png", dpi=120, bbox_inches="tight"
        )
        plt.close()
        # Plot spike-rate MAE
        try:
            ...
            plt.tight_layout()
            plt.savefig(
                dirs["logs"] / "nextstep_mae_rate_vs_context.png",
                dpi=120,
                bbox_inches="tight",
            )
            plt.close()
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(
                dirs["logs"] / "nextstep_relative_vs_context.png",
                dpi=120,
                bbox_inches="tight",
            )
            plt.close()
    ...
    try:
        ...
        if offline_mode and load_from and Path(load_from).exists():
            ...
            with h5py.File(load_from, "r") as f:
                if "truth_next_step" in f and "pred_next_step" in f:
                    ...
                    nm = (
                        torch.from_numpy(f["neuron_mask"][()])
                        if "neuron_mask" in f
                        else None
                    )
                    if nm is not None:
                        ...
                        Lm = t.shape[1]
                        mask_rep = (
                            nm[:, None, :].expand(B, Lm, nm.shape[-1]).to(t.dtype)
                        )
                        denom = mask_rep.sum(dim=-1).clamp_min(1)
                        truth_means_list.append(
                            ((t * mask_rep).sum(dim=-1) / denom).detach().cpu()
                        )
                        pred_means_list.append(
                            ((p * mask_rep).sum(dim=-1) / denom).detach().cpu()
                        )
                    else:
            ...
            with torch.no_grad():
                for batch in tqdm(ns_loader, desc="Embed next-step"):
                    ...
                    denom = mask_rep.sum(dim=-1).clamp_min(1)
                    truth_means_list.append(
                        ((x_tgt.float() * mask_rep).sum(dim=-1) / denom).detach().cpu()
                    )
                    pred_means_list.append(
                        ((pred_mean.float() * mask_rep).sum(dim=-1) / denom)
                        .detach()
                        .cpu()
                    )
        ...
        if all_truth:
            truth_np = torch.cat(
                [t.reshape(-1, t.shape[-1]) for t in all_truth], dim=0
            ).numpy()
            pred_np = torch.cat(
                [p.reshape(-1, p.shape[-1]) for p in all_pred], dim=0
            ).numpy()
            ...
            axs[0].scatter(
                truth_pca[:, 0], truth_pca[:, 1], s=2, alpha=0.5, label="Truth"
            )
            axs[0].scatter(pred_pca[:, 0], pred_pca[:, 1], s=2, alpha=0.5, label="Pred")
            axs[0].set_title("PCA (2D)")
            axs[0].legend(markerscale=4)
            axs[1].scatter(
                umap_truth[:, 0], umap_truth[:, 1], s=2, alpha=0.5, label="Truth"
            )
            axs[1].scatter(
                umap_pred[:, 0], umap_pred[:, 1], s=2, alpha=0.5, label="Pred"
            )
            axs[1].set_title("UMAP (2D)")
            axs[1].legend(markerscale=4)
            plt.tight_layout()
            plt.savefig(
                dirs["logs"] / "embedding_pca_umap.png", dpi=120, bbox_inches="tight"
            )
            plt.close(fig)

            # Mean activation comparison using masked means (scatter over all timepoints)
            truth_mean = torch.cat(
                [t.reshape(-1) for t in truth_means_list], dim=0
            ).numpy()
            pred_mean = torch.cat(
                [p.reshape(-1) for p in pred_means_list], dim=0
            ).numpy()
            ...
            plt.figure(figsize=(6, 4))
            plt.scatter(
                truth_sorted, pred_sorted, s=6, alpha=0.6, label="Predicted vs True"
            )
            ...
            plt.tight_layout()
            plt.savefig(
                dirs["logs"] / "mean_activation_nextstep.png",
                dpi=120,
                bbox_inches="tight",
            )
            plt.close()
            ...
            try:
                ...
                if true_rates_sorted.size > 1:
                    r_val = float(
                        np.corrcoef(true_rates_sorted, pred_rates_sorted)[0, 1]
                    )
                else:
                    r_val = float("nan")
                plt.figure(figsize=(6, 4))
                ...
                plt.tight_layout()
                plt.savefig(
                    dirs["logs"] / "calibration_true_vs_predicted_rates.png",
                    dpi=120,
                    bbox_inches="tight",
                )
                plt.close()
