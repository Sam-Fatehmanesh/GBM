    ...
    try:
        ...
        with open(loss_txt, "r") as f:
            for line in f:
                ...
                m = re.search(
                    r"epoch\s+(\d+),\s*step\s+(\d+),\s*train\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?),\s*val\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)",
                    line,
                )
                if m:
                    rows.append(
...
@torch.no_grad()
def _apply_spatial_voxel_blur_torch(
    spikes_LN: np.ndarray,
...
def _make_blur_visualization(
    ...
    for i, bv in enumerate(blur_levels, start=2):
        blurred = _apply_spatial_voxel_blur_torch(
            spikes_TN[t : t + 1, :], positions_N3, mask, bv
        )[0]
        ...
        lim2 = [
            ...
            max(
                1e-6,
                float(np.percentile(np.concatenate([base[idxs], blurred[idxs]]), 99.5)),
            ),
        ]
    ...
    for bv in blur_levels:
        ...
        try:
            ...
            writer = imageio.get_writer(
                out_path, fps=int(fps), codec="libx264", format="FFMPEG"
            )
        except Exception:
            ...
            try:
                writer = imageio.get_writer(
                    out_path, duration=1.0 / float(fps), loop=0, format="GIF"
                )
            except Exception:
                writer = None
        for t in idxs:
            orig = spikes_TN[t]
            blurred = _apply_spatial_voxel_blur_torch(
                orig[None, :], positions_N3, mask, float(bv)
            )[0]
            grid_o = _rasterize_to_grid(
                orig, positions_N3, plane=plane, grid_hw=grid_hw
            )
            grid_b = _rasterize_to_grid(
                blurred, positions_N3, plane=plane, grid_hw=grid_hw
            )
            # map to 0..1
...
def _run_training_once(
    train_script: Path, cfg: Dict[str, Any], tmp_cfg_path: Path
) -> Path | None:
...
def _sweep(config: Dict[str, Any]) -> None:
    ...
    blur_levels: List[float] = [
        float(x) for x in sweep_cfg.get("blur_levels", [0.0, 2.0])
    ]
    ...
    train_script = _repo_root() / (
        "GenerativeBrainModel/scripts/train_gbm2_unblur_targets.py"
        if use_unblur_targets
        else "GenerativeBrainModel/scripts/train_gbm2.py"
    )

    # Pre-training blur visualizations (scatter + videos)
    data_dir_vis = Path(
        train_overrides.get("data", {}).get("data_dir", "processed_spike_voxels_2018")
    )
    ...
    for bv in blur_levels:
        for seed in seeds:
            ...
            cfg_full = _deep_update(
                base_cfg, {"experiment": {"name": f"blur_sweep_b{bv}_s{seed}"}}
            )
            ...
            if len(kept) != 1:
                raise RuntimeError(
                    f"Expected exactly 1 subject after include_files filter, got {len(kept)} in {data_dir_abs}. include_files={include_list}"
                )
            ...
            _write_intraepoch_csv_from_loss(
                run_dir / "logs" / "loss.txt", run_dir / "logs" / "val_intraepoch.csv"
            )
            results.append(
                (
                    float(bv),
                    int(seed),
                    float(last_val) if last_val is not None else float("nan"),
                )
            )
            ...
            _cleanup_cuda_memory()
            sweep_pbar.update(1)
            sweep_pbar.set_postfix(
                {
                    "blur": bv,
                    "seed": seed,
                    "val": (None if last_val is None else float(last_val)),
                }
            )
    ...
    with open(agg_csv, "w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["blur_voxel_size", "final_val_mean", "final_val_std", "n_runs"],
        )
        writer.writeheader()
...
def main():
    ap = argparse.ArgumentParser(
        description="Sweep spatial blur voxel size and measure final validation loss (subject 14)."
    )
    ap.add_argument(
        "--config",
        type=str,
        default="configs/blur_sweep_subject14_test.yaml",
