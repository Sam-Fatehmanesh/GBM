...
def _subject_behavior_dim(subject_h5: Path) -> int:
    with h5py.File(subject_h5, "r") as f:
        ...
        if beh.ndim == 1:
            ...
            raise ValueError(
                f"Unexpected behavior_full ndim={beh.ndim} in {subject_h5}"
            )
...
def load_bmodel_from_checkpoint(
    ckpt_path: Path, device: torch.device
) -> Tuple[BModel, Dict[str, Any]]:
    ...
    model = BModel(
        d_behavior=1,
        d_max_neurons=int(d_max_neurons) if d_max_neurons is not None else 0,
    )
    model.to(device)
    model.eval()
    ...
    try:
        ...
        model.load_state_dict(
            {k: v.cpu() if isinstance(v, torch.Tensor) else v for k, v in sd.items()},
            strict=False,
        )
    return model, cfg_in


def build_subject_loader(
    data_dir: Path, subject: str, batch_size: int, num_workers: int, use_cache: bool
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate BModel behavior predictors and visualize PCA"
    )
    parser.add_argument(
    ...
    if not target_subject:
        raise ValueError(
            "data.target_subject must be specified in the config (e.g., subject_14)"
        )
    ...
    if disable_preds:
        dims_to_use = []
        logger.info(
            "Predictions disabled: will only compute PCA on original behavior and skip model/random generations."
        )
    else:
        dims_to_use = [d for d in all_dim_ids if d < K_subject]
        if not dims_to_use:
            raise ValueError(
                f"No overlapping dims to evaluate (subject has {K_subject} dims, available models: {all_dim_ids})"
            )
        logger.info(
            f"Evaluating dims 0..{len(dims_to_use) - 1} (subject K={K_subject}, available models={len(all_dim_ids)})"
        )
    ...
    if not disable_preds:
        for d in dims_to_use:
            ...
            if last_ckpt is None:
                logger.warning(
                    f"No epoch_* checkpoint found in {dim_dir}; skipping dim {d}"
                )
                continue
    ...
    val_loader = build_subject_loader(
        data_dir,
        target_subject,
        cfg["eval"]["batch_size"],
        ...
        for batch in tqdm(val_loader, desc="Predict subject behaviors"):
            ...
            if not disable_preds:
                ...
                for d, m in dim_models:
                    out = (
                        m(spikes, positions, mask, get_logits=True)
                        .squeeze(1)
                        .squeeze(-1)
                    )  # (B,1,1)->(B,)
                    if clamp_pred:
    ...
    pca, originals_2d = fit_pca_2d(
        originals_np, random_state=int(cfg["eval"].get("random_seed", 42))
    )
    pred_2d = (
        pca.transform(predictions_np)
        if (not disable_preds and predictions_np.size > 0)
    ...
    if first_positions is None or first_mask is None or seq_len_L is None:
        raise RuntimeError(
            "Failed to capture positions/mask/sequence length from loader."
        )
    if disable_preds:
        ...
        try:
            ...
            plt.scatter(
                originals_2d[:, 0], originals_2d[:, 1], s=4, alpha=0.5, label="Original"
            )
            plt.title(f"Behavior PCA (subject={target_subject})")
            plt.legend(markerscale=3)
            plt.tight_layout()
            plt.savefig(
                dirs["logs"] / "behavior_pca_scatter.png", dpi=140, bbox_inches="tight"
            )
            plt.close()
    ...
    rand_bs = int(
        cfg["eval"].get("random_batch_size", cfg["eval"].get("batch_size", 128))
    )
    ...
    if (
        isinstance(rb, dict)
        and (rb.get("mean") is not None)
        and (rb.get("var") is not None)
    ):
        m = float(rb["mean"])
        v = float(rb["var"])
        eps = 1e-9
    ...
    with torch.no_grad():
        for start in tqdm(
            range(0, rand_total, rand_bs), desc="Random generation (chunks)"
        ):
            ...
            try:
                ...
                if global_beta:
                    ...
                    spikes_rand = dist.sample(
                        (b, seq_len_L)
                    )  # broadcasts per-neuron (N)
                spikes_rand = spikes_rand.clamp(0.0, 1.0)
            except Exception:
                # Fallback to uniform if Beta fails
                spikes_rand = torch.rand(
                    (b, seq_len_L, Npad), device=device, dtype=torch.float32
                )
            ...
            for d, m in dim_models:
                out = (
                    m(spikes_rand, pos_rand, mask_rand, get_logits=True)
                    .squeeze(1)
                    .squeeze(-1)
                )
                if clamp_pred:
    ...
    if (not disable_preds) and bool(cfg["eval"].get("behavior_first", False)):
        ...
        if first_spikes_window is None:
            ...
            base_win_cpu = torch.zeros(
                (int(seq_len_L), Npad_local), dtype=torch.float32
            )
        else:
        ...
        for start in tqdm(
            range(0, num_samples, bsz), desc="Behavior-first (optimize spikes)"
        ):
            ...
            delta_param = torch.zeros(
                (m, Npad), device=device, dtype=torch.float32, requires_grad=True
            )
            ...
            for _ in range(steps):
                ...
                spikes_const = torch.clamp(
                    base_rep + delta_now.unsqueeze(1).repeat(1, Lw, 1), 0.0, 1.0
                )
                total_loss = 0.0
                for d_idx, (d, mdl) in enumerate(dim_models):
                    out = (
                        mdl(spikes_const, pos_b, mask_b, get_logits=True)
                        .squeeze(1)
                        .squeeze(-1)
                    )
                ...
                target_vec = torch.full(
                    (m,), target_frame_mean, device=device, dtype=spikes_const.dtype
                )
            ...
            with torch.no_grad():
                ...
                probs_final = torch.clamp(
                    base_win.unsqueeze(0).repeat(m, 1, 1)
                    + delta_final.unsqueeze(1).repeat(1, Lw, 1),
                    0.0,
                    1.0,
                )
                ...
                for d_idx, (d, mdl) in enumerate(dim_models):
                    out = (
                        mdl(spikes_final, pos_b, mask_b, get_logits=True)
                        .squeeze(1)
                        .squeeze(-1)
                    )
                    pred_dims.append(out.clamp(0.0, 1.0))
                pred_mat = (
                    torch.stack(pred_dims, dim=1).detach().cpu().numpy()
                )  # (m, K_use)
                ...
                collected_modified_win.append(
                    spikes_final.detach().cpu().numpy()
                )  # (m,L,N)
                if trace:
        ...
        if collected_novel:
            ...
            try:
                ...
                deltas_all = (
                    np.concatenate(collected_deltas, axis=0)
                    if collected_deltas
                    else np.zeros((0, int(first_positions.shape[0])), dtype=np.float32)
                )
        ...
        try:
            if loss_traces:
                ...
                plt.tight_layout()
                plt.savefig(
                    dirs["logs"] / "behavior_opt_loss.png", dpi=140, bbox_inches="tight"
                )
                plt.close()
        except Exception:
            pass
    ...
    try:
        ...
        if gen_2d.shape[0] > 0 and bool(
            cfg["eval"].get("enable_random_generation", False)
        ):
            plt.scatter(
                gen_2d[:, 0], gen_2d[:, 1], s=4, alpha=0.5, label="Random-generated"
            )
