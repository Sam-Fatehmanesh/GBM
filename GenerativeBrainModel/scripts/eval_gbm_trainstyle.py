...
from GenerativeBrainModel.scripts.eval_gbm import (
    ...
    try:
        ...
        if (
            ...
            cfg["eval"]["sequence_length"] = tr_cfg.get(
                "sequence_length", cfg["eval"].get("sequence_length")
            )
        ...
        if (
            (not cfg["data"].get("test_subjects"))
            and isinstance(data_cfg, dict)
            and data_cfg.get("test_subjects")
        ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate GBM using train-style aggregated validation"
    )
    parser.add_argument(
    ...
    if not ckpt or not Path(ckpt).exists():
        raise ValueError(
            "Please specify model.checkpoint (path to checkpoint saved by train_gbm.py)"
        )
