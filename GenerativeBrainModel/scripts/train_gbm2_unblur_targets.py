...
from GenerativeBrainModel.dataloaders.single_subject_dataloader import (
    create_single_subject_dataloaders,
)
...
def _apply_spatial_voxel_blur(
    spikes: torch.Tensor, positions: torch.Tensor, mask: torch.Tensor, voxel_size: float
) -> torch.Tensor:
...
def main():
    ap = argparse.ArgumentParser(
        description="Train GBM with blurred inputs and UNBLURRED targets"
    )
    ap.add_argument(
    ...
    train_loader, val_loader, _, _, unique_neuron_ids = (
        create_single_subject_dataloaders(cfg)
    )
    ...
    def train_or_val_loop(loader, epoch: int, train: bool) -> float:
        ...
        for batch in pbar:
            ...
            spikes_blur = (
                _apply_spatial_voxel_blur(spikes, positions, mask, blur_vox)
                if blur_vox > 0.0
                else spikes
            )
                ...
                try:
                    ...
                    with torch.no_grad():
                        for vbatch in val_loader:
                            ...
                            blur_vox_v = float(
                                cfg["data"].get("spatial_blur_voxel_size", 0.0) or 0.0
                            )
                            spikes_blur_v = (
                                _apply_spatial_voxel_blur(
                                    spikes_v, positions_v, mask_v, blur_vox_v
                                )
                                if blur_vox_v > 0.0
                                else spikes_v
                            )
