...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        optimizer.zero_grad()
        preds = model(x_use, pos_use, mask_use, get_logits=True).squeeze(1)    # (b_bal, 1)
...
@torch.no_grad()
def validate(model: BModel, loader, device: torch.device, epoch: int, logger: logging.Logger, *, target_dim: int = 0, pos_weight: Optional[torch.Tensor] = None) -> Tuple[float, Optional[Dict[str, np.ndarray]]]:
    model.eval()
    loss_fn = nn.L1Loss()
    total_loss = 0.0
    total_batches = 0
    ...
    for i, batch in enumerate(tqdm(loader, desc=f"Val E{epoch}")):
        ...
        preds = model(spikes, positions, batch.get('neuron_mask', None), get_logits=True).squeeze(1)
        target = behavior[:, target_dim:target_dim+1]
...
def plot_pred_vs_true_dim(sample: Dict[str, np.ndarray], plots_dir: Path, epoch: int, dim: int) -> None:
...
@torch.no_grad()
def evaluate_subjectwise(model: BModel, loader, device: torch.device, *, target_dim: int, loss_fn: nn.Module) -> list[dict]:
    ...
    for batch in tqdm(loader, desc="Per-subject eval"):
        ...
        sample_loss = torch.mean(torch.abs(preds.float() - target.float()), dim=1).detach().cpu().numpy()
...
def main():
    ...
    for dim in range(d_behavior):
        ...
        optimizer = optim.AdamW(model.parameters(), lr=base_lr, weight_decay=cfg['training']['weight_decay'])

        total_train_steps = len(train_loader) * int(cfg['training']['num_epochs'])
        warmup_steps = int(float(cfg['training'].get('lr_warmup_ratio', 0.0)) * max(1, total_train_steps))
        def lr_lambda(step: int) -> float:
            ...
            return 1.0
        scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lr_lambda) if warmup_steps > 0 else None
        ...
        for epoch in range(1, cfg['training']['num_epochs'] + 1):
            ...
            def train_with_weighted_bce():
                return train_one_epoch(
                    model, train_loader, device, optimizer, epoch, logger, train_history, global_step,
                    scheduler=scheduler, clip_norm=clip, target_dim=dim, pos_weight=None
                )
            _, global_step = train_with_weighted_bce()
            # Validate (BCE with logits; same pos_weight)
            val_loss, sample = validate(model, val_loader, device, epoch, logger, target_dim=dim, pos_weight=None)
            if sample is not None:
                plot_pred_vs_true_dim(sample, dim_plots, epoch, dim)
            plot_training_loss(train_history, dim_plots, dirs['logs'])
            if val_loss < best_val:
                best_val = val_loss
                torch.save({'epoch': epoch, 'model': model.state_dict(), 'config': cfg, 'dim': dim}, dim_ckpt / 'best.pth')
            torch.save({'epoch': epoch, 'model': model.state_dict(), 'config': cfg, 'dim': dim}, dim_ckpt / f'epoch_{epoch}.pth')

        # After training this dim model, run subject-wise evaluation over both train and validation loaders and save CSV
        try:
            import csv
            # Train split
            rows_tr = evaluate_subjectwise(model, train_loader, device, target_dim=dim, loss_fn=nn.L1Loss())
            csv_path_tr = dim_plots / f'subject_eval_train_dim{dim}.csv'
            with open(csv_path_tr, 'w', newline='') as f:
                w = csv.DictWriter(f, fieldnames=['file_path', 'num_samples', 'loss_mean', 'corr'])
                w.writeheader()
                for r in rows_tr:
                    w.writerow(r)
            # Validation split
            rows_va = evaluate_subjectwise(model, val_loader, device, target_dim=dim, loss_fn=nn.L1Loss())
