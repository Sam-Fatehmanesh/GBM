...
def _load_state_dict_robust(model: torch.nn.Module, state: dict) -> None:
    ...
    if len(filtered) == 0:
        raise RuntimeError(
            "No matching parameters found between checkpoint and current model. Ensure code version matches the checkpoint."
        )
    model.load_state_dict(filtered, strict=False)
    try:
        print(
            f"[eval] loaded params: matched={len(filtered)} missing={len(missing)} unexpected={len(unexpected)}"
        )
    except Exception:
        pass
...
@torch.no_grad()
def make_log_true_vs_log_sigma_scatter(
    step_dir: Path,
...
@torch.no_grad()
def make_log_true_vs_log_mu_scatter(
    step_dir: Path,
    true_last: torch.Tensor,  # (N,) rate
    ...
    fig, ax = plt.subplots(1, 1, figsize=(6, 6))
    hb = ax.hexbin(
        xs, ys, gridsize=80, extent=(lo, hi, lo, hi), cmap="magma", bins="log", mincnt=1
    )
...
@torch.no_grad()
def make_mean_rate_plots_stacked(
    step_dir: Path,
    ...
    for i in range(K):
        ...
        if ctx_mean.size:
            ax.plot(
                x_context, ctx_mean, color="gray", lw=1.2, label="context mean rate"
            )
        if fut_mean.size:
            ax.plot(
                x_future, fut_mean, color="tab:blue", lw=1.2, label="future truth mean"
            )
        if pred_mean.size:
            ax.plot(
                x_future,
...
@torch.no_grad()
def make_median_rate_plots_stacked(
    step_dir: Path,
    contexts: list[torch.Tensor],
    futures: list[torch.Tensor],
    preds: list[torch.Tensor],
    ...
    for i in range(K):
        ...
        if ctx_med.size:
            ax.plot(
                x_context, ctx_med, color="gray", lw=1.2, label="context median rate"
            )
        if fut_med.size:
            ax.plot(
                x_future, fut_med, color="tab:blue", lw=1.2, label="future truth median"
            )
        if pred_med.size:
            ax.plot(
                x_future,
...
@torch.no_grad()
def make_spike_counts_plots_stacked(
    step_dir: Path,
    ...
    for i in range(K):
        ...
        if Tc > 0:
            ax.plot(
                x_context,
                ctx_counts.to(torch.float32).cpu().numpy(),
                color="gray",
                lw=1.2,
                label="context spike count",
            )
        if Tf > 0 and true_counts.numel() == Tf:
            ax.plot(
                x_future,
                true_counts.to(torch.float32).cpu().numpy(),
                color="tab:blue",
                lw=1.2,
                label="future truth spike count",
            )
        if Tf > 0:
            ax.plot(
                x_future,
                pred_counts.to(torch.float32).cpu().numpy(),
                color="tab:orange",
                lw=1.2,
                label="future pred spike count",
            )
        if i == 0:
            ax.legend(loc="upper right", fontsize=8)
        ax.set_ylabel(f"AR {i + 1}")
...
def make_spike_counts_plot(
    ...
    if Tf > 0 and future_truth_counts.numel() == Tf:
        ax.plot(
            x_future,
            future_truth_counts.to(torch.float32).cpu().numpy(),
            color="tab:blue",
            lw=1.5,
            label="future truth spike count",
        )
    if Tf > 0:
        ax.plot(
            x_future,
            future_pred_counts.to(torch.float32).cpu().numpy(),
            color="tab:orange",
            lw=1.5,
            label="future pred spike count",
        )
...
@torch.no_grad()
def make_per_index_loss_plot(
    step_dir: Path, per_index_mean: np.ndarray, title: str
) -> None:
...
def make_pca_population_plot(
    ...
    if len(true_list) == 0 or len(pred_list) == 0:
        _dbg(
            f"no samples: len(true_list)={len(true_list)}, len(pred_list)={len(pred_list)}"
        )
        return
    try:
    ...
    Xpred_imp = torch.where(
        Mkeep, Xpred, col_mean.unsqueeze(0)
    )  # use same mean for pred
...
@torch.no_grad()
def make_pca_population_plot_per_subject(
    step_dir: Path,
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(3.2 * cols, 3.2 * rows), sharex=True, sharey=True
    )
    ...
    for i in range(rows * cols):
        ...
        if i < S_eff:
            ...
            ax.set_title(
                subjects[i] if i < len(subjects) else f"subj_{i + 1}", fontsize=8
            )
        ax.set_xlim(lo[0], hi[0])
        ax.set_ylim(lo[1], hi[1])
        if r == rows - 1:
...
@torch.no_grad()
def make_laststep_pca_grid_per_subject(
    step_dir: Path, subj_to_true: dict, subj_to_pred: dict, subj_to_masks: dict
) -> None:
    ...
    fig, axes = plt.subplots(
        rows, cols, figsize=(3.6 * cols, 3.6 * rows), sharex=False, sharey=False
    )
    axes = np.array(axes).reshape(rows, cols)
    for i, subj in enumerate(names):
        ...
        try:
            ...
            X_pred = (
                torch.stack([p.to(torch.float32) for p in pl], dim=0)
                if len(pl) == len(tl)
                else None
            )
            M = (
                torch.stack([m.to(torch.bool) for m in ml], dim=0)
                if len(ml) == len(tl)
                else torch.ones_like(X_true, dtype=torch.bool)
            )
            keep = M.all(dim=0)
            ...
            Zt = Xt_c @ comps
            ax.scatter(
                Zt[:, 0], Zt[:, 1], s=10, alpha=0.7, color="tab:blue", label="true"
            )
            if Xp_c is not None:
                Zp = Xp_c @ comps
                ax.scatter(
...
def autoregressive_rollout(
    ...
    spike_prob_init = torch.nan_to_num(
        spike_prob_init, nan=0.0, posinf=1.0, neginf=0.0
    ).clamp_(0.0, 1.0)
    spike_mask_ctx = torch.bernoulli(spike_prob_init).to(
        torch.float32
    )  # (1, Tc, N) 0/1 floats
    preds = []
    # Precompute context spike counts per timestep (sum over neurons)
    ctx_counts = (
        spike_mask_ctx[:, : int(init_context.shape[1]), :]
        .sum(dim=2)
        .to(torch.float32)[0]
    )  # (Tc,)
    ...
    for t in range(Tf):
        ...
        spike_probs_window = spike_mask_ctx[
            :, -win_len:, :
        ]  # (1, win_len, N) float32 0/1
    ...
    return (
        ...
        ctx_counts,
        torch.stack(pred_counts, dim=0),
    )  # (Tf,N), (Tc,), (Tf,)


def main():
    ap = argparse.ArgumentParser(
        description="Evaluate GBM (validation + AR) from an experiment directory"
    )
    ap.add_argument(
        "--exp_dir",
        type=str,
        required=True,
        help="Path to experiments/gbm2/<YYYYmmdd_HHMMSS>",
    )
    ap.add_argument(
        "--ckpt",
        type=str,
        default=None,
        help="Optional checkpoint path; defaults to <exp_dir>/checkpoints/best_model.pth",
    )
    ap.add_argument(
        "--val_batches",
        type=int,
        default=None,
        help="Number of validation batches to average (defaults to cfg.training.val_sample_batches or all)",
    )
    ap.add_argument(
        "--horizon",
        type=int,
        default=64,
        help="AR prediction horizon steps for visualization",
    )
    ap.add_argument(
        "--num_ars",
        type=int,
        default=16,
        help="Number of unique AR rollouts to run and visualize",
    )
    args = ap.parse_args()
    ...
    ckpt_path = (
        Path(args.ckpt) if args.ckpt else (exp_dir / "checkpoints" / "best_model.pth")
    )
    ...
    val_batches_limit = (
        ...
        else int(cfg["training"].get("val_sample_batches") or 0)
    )
    ...
    with torch.no_grad():
        ...
        for batch in val_loader:
            ...
            if lam.numel() > 0 and las.numel() > 0:
                mu_z = (mu.to(torch.float32) - lam_e2.to(torch.float32)) / las_e2.to(
                    torch.float32
                )
            ...
            nll = (
                0.5 * z_err.pow(2) + torch.log(sigma_z) + 0.5 * math.log(2.0 * math.pi)
            )
            ...
            for bi in range(B):
                ...
                pred_med = lognormal_rate_median(
                    mu[bi : bi + 1, -1:, :].detach().cpu(),
                    raw_log_sigma[bi : bi + 1, -1:, :].detach().cpu(),
                )[0, 0, :]
            ...
            if len(ar_contexts) < num_ars:
                ...
                for bi in range(B):
                    ...
                    pred_future, ctx_spike_counts, pred_spike_counts = (
                    ...
                    try:
                        ...
                        ctx_rates = (
                            init_context[0].to(torch.float32).cpu().numpy()
                        )  # (Tc,N)
                        fut_truth = (
                            x_in_full[bi, Tc : Tc + Tf_all, :]
                            .to(torch.float32)
                            .cpu()
                            .numpy()
                        )  # (Tf,N)
                        ...
                        stim_ctx = (
                            stim[bi, :Tc, :].to(torch.float32).cpu().numpy()
                        )  # (Tc,K)
                        stim_fut = (
                            stim[bi, Tc : Tc + Tf_all, :]
                            .to(torch.float32)
                            .cpu()
                            .numpy()
                        )  # (Tf,K)
                        stim_all = (
                            stim[bi, : Tc + Tf_all, :].to(torch.float32).cpu().numpy()
                        )  # (Tc+Tf,K)
                        ...
                        lam_np = (
                            lam[bi].to(torch.float32).cpu().numpy()
                            if lam.numel() > 0
                            else None
                        )
                        las_np = (
                            las[bi].to(torch.float32).cpu().numpy()
                            if las.numel() > 0
                            else None
                        )
                        ctx_counts_np = (
                            ctx_spike_counts.to(torch.float32).cpu().numpy()
                        )  # (Tc,)
                        # compute true-future spike counts for this AR window
                        prob_true_local = 1.0 - torch.exp(
                            -x_in_full[bi, Tc : Tc + Tf_all, :].to(torch.float32) / sr
                        )
                        prob_true_local = torch.nan_to_num(
                            prob_true_local, nan=0.0, posinf=1.0, neginf=0.0
                        ).clamp_(0.0, 1.0)
                        true_mask_local = torch.bernoulli(prob_true_local).to(
                            torch.float32
                        )
                        true_counts_np = (
                            true_mask_local.sum(dim=1).to(torch.float32).cpu().numpy()
                        )  # (Tf,)
                        pred_counts_np = (
                            pred_spike_counts.to(torch.float32).cpu().numpy()
                        )  # (Tf,)
                        ...
                        with h5py.File(str(out_path), "w") as hf:
                            ...
                            # Counts
                            hf.create_dataset(
                                "context_spike_counts",
                                data=ctx_counts_np,
                                compression="gzip",
                                compression_opts=2,
                            )
                            hf.create_dataset(
                                "future_truth_spike_counts",
                                data=true_counts_np,
                                compression="gzip",
                                compression_opts=2,
                            )
                            hf.create_dataset(
                                "future_pred_spike_counts",
                                data=pred_counts_np,
                                compression="gzip",
                                compression_opts=2,
                            )
                            ...
                            if start_idx_vec is not None:
                                try:
                                    hf.attrs["start_idx"] = int(
                                        start_idx_vec[bi].item()
                                    )
                                except Exception:
                                    pass
                            hf.attrs["Tc"] = int(Tc)
                        ...
                        try:
                            print(
                                f"[eval_ar] Warning: failed to save AR run to {out_path}: {_e}"
                            )
                        except Exception:
                            pass
                    ...
                    if final_ctx is None:
                        ...
                        prob_true_first = 1.0 - torch.exp(
                            -final_truth.to(torch.float32) / sr
                        )
                        prob_true_first = torch.nan_to_num(
                            prob_true_first, nan=0.0, posinf=1.0, neginf=0.0
                        ).clamp_(0.0, 1.0)
                        true_future_mask_first = torch.bernoulli(prob_true_first).to(
                            torch.float32
                        )
                    ...
                    prob_true = torch.nan_to_num(
                        prob_true, nan=0.0, posinf=1.0, neginf=0.0
                    ).clamp_(0.0, 1.0)
                    ...
                    pred_last = sample_lognormal(
                        mu[bi : bi + 1, -1:, :], raw_log_sigma[bi : bi + 1, -1:, :]
                    )[0, 0, :]
                    mu_last = mu[bi, -1, :].to(torch.float32)
                    sigma_last = (
                        F.softplus(raw_log_sigma[bi, -1, :].to(torch.float32)) + 1e-6
                    )
                    ...
                    try:
                        ...
                        subj = (
                            _P(fp).name
                            if fp is not None
                            else f"subj_{len(scat_subjects) + 1}"
                        )
                    except Exception:
                    ...
                    subj_pred = lognormal_rate_median(
                        mu[bi, :Lm1_use, :].detach().cpu(),
                        raw_log_sigma[bi, :Lm1_use, :].detach().cpu(),
                    )
    ...
    if final_ctx is not None and final_truth is not None and final_pred is not None:
        make_validation_plots(
            ...
            final_truth,
            final_pred,
            title=f"Eval autoreg 16 neurons",
        )
        make_mean_rate_plot(
            step_dir,
            final_ctx,
            final_truth,
            final_pred,
            title=f"Eval mean rate over time (first AR)",
        )
        make_median_rate_plot(
            ...
            final_truth,
            final_pred,
            title=f"Eval median rate over time (first AR)",
        )
        if (
            first_ctx_counts is not None
            and first_true_counts is not None
            and first_pred_counts is not None
        ):
            make_spike_counts_plot(
                step_dir,
                first_ctx_counts,
                first_true_counts,
                first_pred_counts,
                title=f"Eval spike counts over time (first AR)",
            )
    # Stacked over-time plots across multiple AR runs
    if len(ar_contexts) > 0:
        make_mean_rate_plots_stacked(
            step_dir,
            ar_contexts,
            ar_truths,
            ar_preds,
            title=f"Eval mean rate over time (stacked {len(ar_contexts)} ARs)",
        )
        make_median_rate_plots_stacked(
            step_dir,
            ar_contexts,
            ar_truths,
            ar_preds,
            title=f"Eval median rate over time (stacked {len(ar_contexts)} ARs)",
        )
        make_spike_counts_plots_stacked(
            step_dir,
            ar_ctx_counts,
            ar_true_counts,
            ar_pred_counts,
            title=f"Eval spike counts over time (stacked {len(ar_contexts)} ARs)",
        )

        # Ensembled scatter/density plots (concatenate across runs)
        true_all = (
            torch.cat(scat_true_last, dim=0) if len(scat_true_last) else torch.empty(0)
        )
        pred_all = (
            torch.cat(scat_pred_last, dim=0) if len(scat_pred_last) else torch.empty(0)
        )
        mu_all = torch.cat(scat_mu_last, dim=0) if len(scat_mu_last) else torch.empty(0)
        sigma_all = (
            torch.cat(scat_sigma_last, dim=0)
            if len(scat_sigma_last)
            else torch.empty(0)
        )
        mask_all = (
            torch.cat(scat_mask_last, dim=0)
            if len(scat_mask_last)
            else torch.empty(0, dtype=torch.bool)
        )
        if true_all.numel() > 0:
            make_next_step_scatter(
                step_dir,
                true_all,
                pred_all,
                mask_all,
                title=f"Eval next-step scatter (ensembled {len(scat_true_last)} ARs)",
            )
            make_mu_sigma_scatter(
                step_dir,
                mu_all,
                sigma_all,
                true_all,
                mask_all,
                title=f"Eval mu vs sigma (ensembled {len(scat_true_last)} ARs)",
            )
            make_log_rate_scatter(
                step_dir,
                true_all,
                pred_all,
                mask_all,
                title=f"Eval log-rate scatter (ensembled {len(scat_true_last)} ARs)",
            )
            make_log_true_vs_log_sigma_scatter(
                step_dir,
                true_all,
                sigma_all,
                mask_all,
                title=f"Eval log(true) vs log(sigma) (ensembled {len(scat_true_last)} ARs)",
            )
            make_log_true_vs_log_mu_scatter(
                step_dir,
                true_all,
                mu_all,
                mask_all,
                title=f"Eval log(true) vs mu (log-rate) (ensembled {len(scat_true_last)} ARs)",
            )
            pass

    # New: Subject-wise PCA using last-step vectors across all batches
    try:
        make_laststep_pca_grid_per_subject(
            step_dir, subj_last_true, subj_last_pred, subj_last_masks
        )
    except Exception as e:
        try:
    ...
    if per_index_count.sum() > 0:
        ...
        make_per_index_loss_plot(
            step_dir, per_idx_mean, title="Mean next-step NLL vs sequence index"
        )

    print(
        f"Eval complete. Val loss: {val_loss:.6f} over {vb_loss_count} batches. Outputs in: {exp_dir}"
    )
