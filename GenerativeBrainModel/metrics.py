...
def pr_auc_binned(predictions: torch.Tensor, targets: torch.Tensor, threshold: float = 0.5, num_bins: int = 1000) -> float:
    """
...
class CombinedMetricsTracker:
    def __init__(self, log_dir: Union[str, Path], ema_alpha: float = 0.1, val_threshold: float = 0.5, enable_plots: bool = True):
        log_dir = Path(log_dir)
        log_dir.mkdir(parents=True, exist_ok=True)
        self.train_csv = CSVLogger(log_dir / 'training.csv', ['epoch', 'batch_idx', 'loss', 'ema_loss', 'lr'])
        self.val_csv = CSVLogger(log_dir / 'validation.csv', ['epoch', 'batch_idx', 'val_loss', 'pr_auc'])
    ...
    def log_validation(self, epoch: int, batch_idx: int, predictions: torch.Tensor, targets: torch.Tensor, val_loss: float) -> Dict[str, float]:
        preds_flat = predictions.detach().flatten()
        targs_flat = targets.detach().flatten()
    ...
    def plot_training(self) -> None:
        ...
        if train_path.exists():
            ...
            if not df.empty:
                x = (df['epoch'] - 1) * max(1, df[df['epoch'] == 1]['batch_idx'].max() or 1) + df['batch_idx']
        ...
        if val_path.exists():
            ...
            if not dfv.empty:
                x = (dfv['epoch'] - 1) * max(1, dfv[dfv['epoch'] == 1]['batch_idx'].max() or 1) + dfv['batch_idx']
