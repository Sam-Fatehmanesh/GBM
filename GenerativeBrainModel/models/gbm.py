...
class GBM(nn.Module):
    def __init__(
        ...
        # Additional stimulus-to-d_model mapping for additive broadcast onto token vectors
        self.stimuli_broadcast_encoder = nn.Sequential(
            nn.Linear(d_stimuli, d_model),
            RMSNorm(d_model),
        )
        # Configure which tokens receive the additive stimulus broadcast.
        # Options: 'all' (default), 'neurons_only', 'none'.
        self.stimulus_add_target: str = "all"
        ...
        self.embed = nn.ModuleDict(
            {
                ...
                "stim_broadcast": self.stimuli_broadcast_encoder,
    ...
    def forward(
        ...
        # Prepare stimulus features in model dtype
        stim_raw = x_stimuli.to(params_dtype)  # (B, T, d_stimuli)
        # (B, T, d_stimuli) -> (B, T, 1, d_model), acts as a global stimulus embedding as a token for each time step
        x_stimuli = self.stimuli_encoder(stim_raw.unsqueeze(2))  # (B, T, 1, d_model)
        ...
        # Add per-timestep stimulus broadcast embedding to token vectors
        stim_add = self.stimuli_broadcast_encoder(stim_raw)  # (B, T, d_model)
        add_bt1d = stim_add.unsqueeze(2)  # (B, T, 1, d_model)
        if self.stimulus_add_target == "neurons_only":
            x[:, :, :N, :] = x[:, :, :N, :] + add_bt1d
        elif self.stimulus_add_target == "all":
            x = x + add_bt1d  # broadcast across token dimension (N+1)
        else:  # 'none' or any other sentinel -> no-op
            pass
