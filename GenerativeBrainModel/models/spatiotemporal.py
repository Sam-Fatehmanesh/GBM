...
class SpatioTemporalNeuralAttention(nn.Module):
    def __init__(self, d_model, n_heads, temporal_gate_k: int | None = None):
        ...
        self.temporal_attention = NeuronCausalAttention(
            d_model=d_model, n_heads=n_heads, capacity_k=temporal_gate_k
        )
    ...
    def forward(
        self, x, point_positions, neuron_pad_mask, neuron_spike_probs, neuron_embed
    ):
        ...
        x = self.temporal_attention(x, neuron_pad_mask, neuron_embed)
        if debug_enabled():
