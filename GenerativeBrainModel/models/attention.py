...
class SpikeSparseConnectomeRoutingAttention(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "input_centroids", input_centroids.to(torch.bfloat16), persistent=True
        )
        self.register_buffer(
            "output_centroids", output_centroids.to(torch.bfloat16), persistent=True
        )
    ...
    def _build_spiking_keep_masks(
        ...
        non_spiking_mask, all_non_spiking = (
            self._get_indicies_of_non_spiking_neurons_in_clusters(
    ...
    @torch.no_grad()
    def _create_initial_centroids(
        self, num_centroids, num_heads, head_dim, position_weight=0.999
    ):
        ...
        input_centroid_features = (
            input_centroid_features / input_centroid_features.norm(dim=-1, keepdim=True)
        )
        output_centroid_features = (
            output_centroid_features
            / output_centroid_features.norm(dim=-1, keepdim=True)
        )
        ...
        z = r * torch.cos(phi)
        centroid_positions = torch.stack(
            [x, y, z], dim=-1
        )  # (num_heads, num_centroids, 3)

        # Final centroids are a weighted concatenation of random initial features and positions
        input_centroids = torch.cat(
            [
                input_centroid_features * (1 - position_weight),
                centroid_positions * position_weight,
            ],
            dim=-1,
        )
        output_centroids = torch.cat(
            [
                output_centroid_features * (1 - position_weight),
                centroid_positions * position_weight,
            ],
            dim=-1,
        )
    ...
    def _calculate_cluster_cosine_scores(self, q, k, unit_point_positions):
        ...
        pos_st = (
            unit_point_positions.unsqueeze(1).expand(B, T, N, 3).reshape(S, N, 3)
        )  # (S,N,3)
        ...
        c_out_feat, c_out_pos = (
            output_centroids_n[..., :Dh],
            output_centroids_n[..., Dh:],
        )

        # Token concat norms: ||[q,pos]|| per (S,H,N,1). Avoid expanding pos over H by broadcasting.
        q_norm2 = (q.to(dtype=torch.float32) ** 2).sum(
            dim=-1, keepdim=True
        )  # (S,H,N,1)
        p_norm2 = (pos_st.to(dtype=torch.float32) ** 2).sum(
            dim=-1, keepdim=True
        )  # (S,N,1)
        p_norm2 = p_norm2.unsqueeze(1)  # (S,1,N,1)
        concat_norm = (
            (q_norm2 + p_norm2).clamp_min(1e-12).sqrt().to(dtype=q.dtype)
        )  # (S,H,N,1)
    ...
    def _compute_input_scores(self, q, unit_point_positions):
        ...
        p_norm2 = (
            (pos_st.to(dtype=torch.float32) ** 2).sum(dim=-1, keepdim=True).unsqueeze(1)
        )
    ...
    @torch.no_grad()
    def _topk_indices_blocked(
        self,
        compute_scores_fn,
        q_or_k: torch.Tensor,
        unit_point_positions: torch.Tensor,
        k: int,
        C: int,
        blocks: int = 4,
    ):
        ...
        for c0 in range(0, C, block_size):
            ...
            if compute_scores_fn is self._compute_input_scores:
                ...
                scores_block = self._compute_input_scores(
                    q_or_k, unit_point_positions
                )  # (S,H,N,block_C)
                self.input_centroids = full
            else:
                full = self.output_centroids
                self.output_centroids = full[:, c0:c1]
                scores_block = self._compute_output_scores(
                    q_or_k, unit_point_positions
                )  # (S,H,N,block_C)
    ...
    @torch.no_grad()
    def _calculate_neuron_top_indices(
        self, input_centroid_cosine_score, output_centroid_cosine_score
    ):
    ...
    @torch.no_grad()
    def _build_cluster_tensors(
        self, q, k, v, input_cluster_neuron_indices, output_cluster_neuron_indices
    ):
        """
        ...
        input_clusters_q = torch.gather(
            ...
            3,
            input_cluster_neuron_indices.unsqueeze(-1).expand(
                -1, -1, -1, -1, D
            ),  # (S,H,C,K,D)
        )
        ...
        output_clusters_k = torch.gather(
            ...
            3,
            output_cluster_neuron_indices.unsqueeze(-1).expand(
                -1, -1, -1, -1, D
            ),  # (S,H,C,K,D)
        )
    ...
    @torch.no_grad()
    def _get_indicies_of_non_spiking_neurons_in_clusters(
        self, output_cluster_neuron_indices, sending_neurons_mask_bt
    ):
        ...
        sending_neurons_mask_expanded = sending_neurons_mask_bt.unsqueeze(1).unsqueeze(
            2
        )  # (B, 1, 1, N)

        # For each item in output_cluster_neuron_indices, gather the mask (True if spiking, False otherwise)
        gathered_mask = torch.gather(
            sending_neurons_mask_expanded.expand(
                B, n_heads, num_clusters_per_head, -1
            ),  # (B, n_heads, num_clusters_per_head, N)
            3,
        ...
        all_non_spiking_in_cluster_mask = non_spiking_mask.all(
            dim=-1
        )  # (B, n_heads, num_clusters_per_head), bool
    ...
    def _ema_update_centroids(
        ...
        def _gather_feats(feats_shnd, idx_shck):
            ...
            if feats_shnd.dim() == 4:  # (S,H,N,Dh)
                ...
                # (S,1,1,N,3) -> (S,H,C,N,3)
                feats_exp = (
                    feats_shnd.unsqueeze(1).unsqueeze(2).expand(S_, H, C, N_, D_)
                )
            return torch.gather(
                feats_exp,
                3,
                idx_shck.unsqueeze(-1).expand(
                    -1, -1, -1, -1, feats_exp.size(-1)
                ),  # (S,H,C,K,D)
            )  # -> (S,H,C,K,D)
    ...
    def forward(self, x, point_positions, neuron_pad_mask, spike_mask):
        ...
        def _mem_ckpt(tag: str):
            if self.profile_memory and torch.cuda.is_available():
                ...
                print(
                    f"[SpikeSparseConnectomeAttention][mem] {tag}: alloc={alloc / 1e9:.2f}GB reserved={reserved / 1e9:.2f}GB"
                )
        ...
        with torch.no_grad():
            ...
            input_idx = self._topk_indices_blocked(
                self._compute_input_scores, q, unit_point_positions, K, C, blocks=4
            )
        ...
        with torch.no_grad():
            output_idx = self._topk_indices_blocked(
                self._compute_output_scores, k, unit_point_positions, K, C, blocks=4
            )
        ...
        if self.training:
            self._ema_update_centroids(
                q,
                k,
                unit_point_positions,  # (S,H,N,Dh), (S,H,N,Dh), (B,N,3)
                input_idx,  # (S,H,C,K)
                output_idx,  # (S,H,C,K)
            )

        # Build cluster tensors (no null yet)
        q_cl, k_cl, v_cl = self._build_cluster_tensors(
            q, k, v, input_idx, output_idx
        )  # (S,H,C,K,Dh)
        ...
        # --------- Spiking masks ----------
        q_keep, kv_keep_real, kv_keep_null, cluster_keep = (
            self._build_spiking_keep_masks(
        ...
        null_k_model = self.k_proj(
            self.null_output_vec.to(device=device, dtype=dtype)
        )  # (Dmodel,)
        null_v_model = self.v_proj(
            self.null_output_vec.to(device=device, dtype=dtype)
        )  # (Dmodel,)
        ...
        null_k_exp = (
            null_k.unsqueeze(0).unsqueeze(2).unsqueeze(3).expand(S, H, C, 1, Dh)
        )
        null_v_exp = (
            null_v.unsqueeze(0).unsqueeze(2).unsqueeze(3).expand(S, H, C, 1, Dh)
        )
        ...
        q_keep_b, kv_keep_b, len_q, len_k, keep_items = (
            self._build_varlen_masks_and_lengths(q_keep, kv_keep, S, H, Cn_q, Cn_k)
        )
        ...
        if B_eff <= max_batch:
            ...
            kv_packed = (
                torch.stack([k_kept, v_kept], dim=1).unsqueeze(2).contiguous()
            )  # (total_k, 2, 1, Dh)
            ...
            counts_lin.index_add_(
                0,
                lin_index,
                torch.ones(lin_index.numel(), device=device, dtype=counts_lin.dtype),
            )
        else:
            ...
            for start in range(0, B_eff, max_batch):
                ...
                kv_packed_chunk = (
                    torch.stack([k_kept_chunk, v_kept_chunk], dim=1)
                    .unsqueeze(2)
                    .contiguous()
                )
                ...
                row_ids_chunk = torch.repeat_interleave(
                    torch.arange(n_seq, device=device), len_q_chunk
                )
                ...
                out_sum_lin.index_add_(
                    0, lin_index_chunk, attn_out_chunk.to(out_sum_lin.dtype)
                )
                counts_lin.index_add_(
                    0,
                    lin_index_chunk,
                    torch.ones(
                        lin_index_chunk.numel(), device=device, dtype=counts_lin.dtype
                    ),
                )
...
class SparseSpikeFullAttention(nn.Module):
    ...
    def forward(self, x, point_positions, neuron_pad_mask, spike_mask):
        ...
        # 4) Build masks and compact indices BEFORE projections (your existing varlen flow)
...
class NeuronCausalAttention(nn.Module):
    ...
    def __init__(
        ...
        self.capacity_k = int(capacity_k) if capacity_k is not None else None
        ...
        # Gating parameters (per-timestep, per-neuron)
        # logit[b,t,n] = gate_vec_state · (x[b,t,n] + state_ctx[b,t])
        # where state_ctx[b,t] = W2(W1(mean_n x[b,t,n])) is a low-rank D->r->D map
        self.gate_vec_state = nn.Parameter(torch.zeros(d_model))
        rank = max(1, d_model // 4)
        self.state_map_1 = nn.Linear(d_model, rank, bias=False)
        self.state_map_2 = nn.Linear(rank, d_model, bias=False)
    ...
    def _apply_timewise_rope_indices(self, x, time_indices: torch.Tensor):
        """
        Apply timewise RoPE using explicit time indices (non-contiguous allowed).
        x: (1, H, L, Dh); time_indices: (L,)
        """
        rotary_dim = self.rotary_dim
        if rotary_dim == 0:
            return x
        idx = time_indices.to(device=x.device)
        sin = self.rope_sin.index_select(0, idx).to(dtype=x.dtype, device=x.device)
        cos = self.rope_cos.index_select(0, idx).to(dtype=x.dtype, device=x.device)
        sin = sin[None, None, :, :]
        cos = cos[None, None, :, :]
    ...
    def forward(
        ...
        neuron_embed: torch.Tensor | None = None,
    ):
        # x: (B, T, N, D)
        B, T, N, D = x.shape
        xn = self.norm(x)
        ...
        # Build per-timestep gating across neurons before attention
        # State-derived context per timestep: mean over neurons, low-rank D->r->D map
        state_ctx = xn.mean(dim=2)  # (B, T, D)
        state_ctx = self.state_map_2(self.state_map_1(state_ctx))  # (B, T, D)
        # Broadcast and compute logits with gate_vec_state
        x_plus_ctx = xn + state_ctx.unsqueeze(2)  # (B, T, N, D)
        logits = torch.einsum(
            "btnd,d->btn", x_plus_ctx.to(x.dtype), self.gate_vec_state.to(x.dtype)
        )
        pad_bool = neuron_pad_mask.to(torch.bool)
        logits = torch.where(
            pad_bool[:, None, :], logits, logits.new_full(logits.shape, float("-inf"))
        )

        K = int(self.capacity_k or 0)
        K = max(0, min(K, int(N)))
        if K > 0:
            if self.gate_temperature > 0.0:
                U = torch.rand_like(logits, dtype=torch.float32)
                gumbel = -torch.log(-torch.log(U.clamp_min(1e-6)))
                logits = logits + gumbel.to(logits.dtype) * float(self.gate_temperature)
            topk_idx = torch.topk(
                logits, k=K, dim=2, largest=True, sorted=False
            ).indices
            sel_mask = torch.zeros_like(logits, dtype=torch.bool)
            sel_mask = sel_mask.scatter(2, topk_idx, True)
        else:
            sel_mask = torch.zeros_like(logits, dtype=torch.bool)

        # Flatten selection to rows (B*N, T)
        sel_mask_rows = sel_mask.permute(0, 2, 1).contiguous().view(B * N, T)
        ...
        if row_indices.numel() > 0 and sel_mask_rows.any():
            qkv_list = []  # (Li, 3, H, Dh)
            cu_seqlens = [0]
            idx_pairs = []  # (row_idx, time_indices)
            max_len = 0
            for ridx in row_indices.tolist():
                time_sel = sel_mask_rows[ridx]
                if not bool(time_sel.any()):
                    continue
                t_idx = torch.nonzero(time_sel, as_tuple=False).squeeze(-1)
                Li = int(t_idx.numel())
                max_len = max(max_len, Li)
                cu_seqlens.append(cu_seqlens[-1] + Li)
                tokens = xt[ridx].index_select(0, t_idx)
                q_row = self.q_proj(tokens)
                k_row = self.k_proj(tokens)
                v_row = self.v_proj(tokens)
                q_row = q_row.view(Li, self.n_heads, self.head_dim)
                k_row = k_row.view(Li, self.n_heads, self.head_dim)
                v_row = v_row.view(Li, self.n_heads, self.head_dim)
                q_bhld = q_row.permute(1, 0, 2).unsqueeze(0).contiguous()
                k_bhld = k_row.permute(1, 0, 2).unsqueeze(0).contiguous()
                q_bhld = self._apply_timewise_rope_indices(q_bhld, t_idx)
                k_bhld = self._apply_timewise_rope_indices(k_bhld, t_idx)
                q_row = q_bhld.squeeze(0).permute(1, 0, 2).contiguous()
                k_row = k_bhld.squeeze(0).permute(1, 0, 2).contiguous()
                qkv = torch.stack([q_row, k_row, v_row], dim=1)
                if qkv.dtype not in (torch.bfloat16, torch.float16):
                    qkv = qkv.to(torch.bfloat16)
                qkv_list.append(qkv)
                idx_pairs.append((ridx, t_idx))
            if len(qkv_list) > 0:
                qkv_packed = torch.cat(qkv_list, dim=0)
                cu = torch.tensor(cu_seqlens, device=x.device, dtype=torch.int32)
                p_drop = self.dropout_p if self.training else 0.0
                attn_packed = flash_varlen_qkv(
                    qkv_packed,
                    cu,
                    int(max_len),
                    float(p_drop),
                ...
                offset = 0
                for (ridx, t_idx), qkv in zip(idx_pairs, qkv_list):
                    Li = int(qkv.shape[0])
                    out_chunk = attn_packed[offset : offset + Li]
                    offset += Li
                    out_chunk_D = out_chunk.reshape(Li, D).to(res.dtype)
                    out[ridx].index_copy_(0, t_idx, out_chunk_D)
