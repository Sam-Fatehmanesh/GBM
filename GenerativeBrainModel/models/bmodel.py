...
class BModel(nn.Module):
    def __init__(self, d_behavior, d_max_neurons, d_hidden: int = 128, fourier_bands: int = 6, include_raw_coords: bool = True):
    ...
    def forward(self, x, point_positions, neuron_mask=None, get_logits=True):
        ...
        if neuron_mask is None:
            mask = torch.ones((B, N), device=point_positions.device, dtype=point_positions.dtype)
        else:
            mask = neuron_mask.to(device=point_positions.device, dtype=point_positions.dtype)
        ...
        freqs = self.fourier_bands.to(device=pos_rel.device, dtype=pos_rel.dtype)  # (nb,)
        pos_exp = pos_rel.unsqueeze(-1) * freqs                                 # (B,N,3,nb)
        sin = torch.sin(pos_exp)
        cos = torch.cos(pos_exp)
        pos_ff = torch.cat([sin, cos], dim=-1).reshape(B, N, 3 * 2 * self.n_bands)  # (B,N,6*nb)
        ...
        var_bt = ((((x - mean_bt) ** 2) * mask_bt).sum(dim=2) / denom_n).unsqueeze(-1)  # (B,T,1)
