...
from GenerativeBrainModel.models.vq import EMACodebook


class VolumeVQVAE(nn.Module):
    """
    ...
    def __init__(self, hidden_channels=512, volume_size=(256, 128, 30), 
                 region_size=(32, 16, 2), num_frequencies=32, sigma=1.0, n_heads=8, codebook_size=512):
        super(VolumeVQVAE, self).__init__()
        ...
        # FFN layers
        self.encoder_FFN = FFN(hidden_channels, hidden_channels*2)
        self.decoder_FFN = FFN(hidden_channels, hidden_channels*2)
        ...
        # VQ VAE Codebook
        self.codebook = EMACodebook(codebook_size, hidden_channels)
    ...
    def forward(self, x):
        ...
        quantized_encoding = self.quantize(encoding)
        
        # Decode 
        reconstruction_logits = self.decode(quantized_encoding, get_logits=True)
                
        return reconstruction_logits, quantized_encoding, encoding
    ...
    def linDepatch(self, x):
        ...
        # Apply RMS normalization before decoder conv (while we still have hidden_channels)
    ...
    def quantize(self, x):
        ...
        Returns:
            Quantized tensor of shape (B, T, n_regions, d_model)
        """
        B, T, n_regions, d_model = x.shape
        
        # Quantize
        return self.codebook.quantize(x)[0]

    def loss(self, quantized_encoding, encoding, reconstruction_logits, target, beta=0.25):
        """
        Compute the loss for the volume vq-vae model.
        
        Args:
            quantized_encoding: Quantized tensor of shape (B, T, n_regions, d_model)
            encoding: Encoded tensor of shape (B, T, n_regions, d_model)
            reconstruction: Reconstructed tensor of shape (B, T, vol_x, vol_y, vol_z)
            target: Target tensor of shape (B, T, vol_x, vol_y, vol_z)
            
        Returns:
            Loss tensor
        """
        # Compute reconstruction loss with BCE
        reconstruction_loss = F.binary_cross_entropy_with_logits(reconstruction_logits, target, reduction='mean')

        # Compute Commitment loss β‖ze(x) − sg[e]‖^2_2
        commitment_loss = beta * F.mse_loss(quantized_encoding.detach(), encoding, reduction='mean')
