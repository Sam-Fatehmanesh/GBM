...
def _render_frame_2d(
    points_xy: np.ndarray, values_uint8: np.ndarray, width: int = 512, height: int = 512
) -> np.ndarray:
    """
...
def create_nextstep_video(
    ...
    try:
        ...
        print(
            f"[VideoGen] nextstep: B={B}, T={T}, N={N}, panel=({panel_width},{panel_height})"
        )
        for b in tqdm(range(B), desc="Video Batches"):
            ...
            if not np.isfinite(xy).all():
                print(
                    f"[VideoGen] NaNs in positions for batch {b}; replacing with zeros."
                )
            ...
            for t in range(T):
                ...
                if not np.isfinite(orig_vals).all() or orig_vals.size == 0:
                    print(
                        f"[VideoGen] orig invalid at b={b} t={t}: shape={orig_vals.shape} minmax={[float(np.nanmin(orig_vals)) if orig_vals.size else None, float(np.nanmax(orig_vals)) if orig_vals.size else None]}"
                    )
                if not np.isfinite(pred_vals).all() or pred_vals.size == 0:
                    print(
                        f"[VideoGen] pred invalid at b={b} t={t}: shape={pred_vals.shape} minmax={[float(np.nanmin(pred_vals)) if pred_vals.size else None, float(np.nanmax(pred_vals)) if pred_vals.size else None]}"
                    )

                # Normalize Truth and Pred together to a shared [0,255] per-frame
                def _scale_to_u8_shared(
                    a: np.ndarray, amin: float, amax: float
                ) -> np.ndarray:
                    if a.size == 0:
                ...
                cv2.putText(
                    frame_bgr, "Truth", (10, 24), font, 0.7, (0, 255, 0), 2, cv2.LINE_AA
                )
                cv2.putText(
                    frame_bgr,
                    "Pred",
...
def create_autoregression_video(
    ...
    frame_size = (
        (panel_width * 2, panel_height) if two_panel else (panel_width, panel_height)
    )
    ...
    try:
        ...
        positions = positions.detach().to(torch.float32).cpu().numpy()
        truth_np = (
            None if truth is None else truth.detach().to(torch.float32).cpu().numpy()
        )
        ...
        for b in tqdm(range(B), desc="AR Video Batches"):
            ...
            for t in range(T):
                ...
                def to_img_with_shared(
                    u: np.ndarray, umin: float, umax: float
                ) -> np.ndarray:
                    if u.size == 0:
                        return np.zeros((panel_height, panel_width), dtype=np.uint8)
                    if (
                        not np.isfinite(umin)
                        or not np.isfinite(umax)
                        or umax <= umin + 1e-12
                    ):
                        return np.zeros((panel_height, panel_width), dtype=np.uint8)
                    u8 = (
                        ((u.astype(np.float32) - umin) / (umax - umin) * 255.0)
                        .clip(0, 255)
                        .astype(np.uint8)
                    )
