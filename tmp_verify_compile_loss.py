...
def collect_batches(loader: torch.utils.data.DataLoader, limit: int) -> List[Dict[str, torch.Tensor]]:
    batches: List[Dict[str, torch.Tensor]] = []
    for idx, batch in enumerate(loader, 1):
        stored = {k: v.clone() if torch.is_tensor(v) else v for k, v in batch.items() if k in {'spikes', 'positions', 'neuron_mask', 'stimulus'}}
...
def compute_mean_loss(model: GBM, batches: List[Dict[str, torch.Tensor]], device: torch.device) -> float:
    losses = []
    with torch.no_grad():
...
def main() -> None:
    ...
    print(f'Collected {len(batches)} batches with batch_size={batches[0]["spikes"].shape[0]}')
