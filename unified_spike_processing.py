    ...
    with PdfPages(output_path) as pdf:
        ...
        if stim is not None:
            try:
                ...
                if hasattr(stim, 'ndim') and int(getattr(stim, 'ndim', 1)) == 2 and stim.shape[1] > 1:
                    # Expect (T, D). Overlay all columns.
                    T_st = stim.shape[0]
                    x = np.arange(T_st)
                    for j in range(stim.shape[1]):
                        ax.plot(x, np.asarray(stim[:, j]).astype(float), lw=0.6, alpha=0.8)
                    ax.set_title(f'Stimuli features (D={stim.shape[1]})')
                    ax.set_ylabel('Feature value')
                else:
                    ax.plot(np.asarray(stim).astype(float), drawstyle='steps-post', color='tab:green', lw=0.8)
        ...
        with h5py.File(final_file, 'w') as f:
            ...
            if config['output'].get('include_additional_data', True):
                ...
                if anat_stack is not None:
                    try:
                        print(f"  → Saving anat_stack (type={type(anat_stack)}, dtype={getattr(anat_stack, 'dtype', None)}, shape={getattr(anat_stack, 'shape', None)})")
                    except Exception:
                        pass
                    try:
                        ...
                        print(f"  → Warning: skipping anat_stack save due to dtype issue: {e}")
                ...
                if stim_full is not None:
                    try:
                        print(f"  → Preparing stimulus_full (type={type(stim_full)}, dtype={getattr(stim_full, 'dtype', None)}, shape={getattr(stim_full, 'shape', None)})")
                    except Exception:
                        pass
                    try:
                    ...
                    try:
                        print(f"  → Saving stimulus_full one-hot (dtype={stim_onehot.dtype}, shape={getattr(stim_onehot, 'shape', None)})")
                        ...
                        print(f"  → Warning: skipping stimulus_full save due to dtype issue: {e}")

                if behavior_full is not None:
                    try:
                        print(f"  → Saving behavior_full (type={type(behavior_full)}, dtype={getattr(behavior_full, 'dtype', None)}, shape={getattr(behavior_full, 'shape', None)})")
                    except Exception:
                        pass
                    try:
                        ...
                        print(f"  → Warning: skipping behavior_full save due to dtype issue: {e}")

                if eye_full is not None:
                    try:
                        print(f"  → Saving eye_full (type={type(eye_full)}, dtype={getattr(eye_full, 'dtype', None)}, shape={getattr(eye_full, 'shape', None)})")
                    except Exception:
                        pass
                    try:
                        ...
                        print(f"  → Warning: skipping eye_full save due to dtype issue: {e}")
            else:
            ...
            _orig_sr_attr = float(_orig_sr_attr) if _orig_sr_attr is not None else float(fpsec)
...
def process_zapbench(config):
    ...
    try:
        ...
        raise ImportError("Reading Zarr v3 requires 'zarrita' (pip install zarrita)") from e

    zap_cfg = config['data'].get('zap', {})
    traces_store = Path(zap_cfg.get('traces_store', '/home/user/gbm3/GBM3/zapdata/traces'))
    seg_store = Path(zap_cfg.get('segmentation_xy_store', '/home/user/gbm3/GBM3/zapdata/segmentation_xy'))
    subject_name = str(zap_cfg.get('subject_name', 'zapbench'))
    # Optional stimuli sources
    stim_store = Path(zap_cfg.get('stimuli_store', '/home/user/gbm3/GBM3/zapdata/stimuli_raw/stimuli_and_ephys.10chFlt'))
    stim_feat_store = Path(zap_cfg.get('stimuli_features_store', '/home/user/gbm3/GBM3/zapdata/stimuli_features'))
    ...
    if stim_feat_store.exists():
        try:
            ...
            if sf.ndim != 2 or sf.shape[1] != 26:
                raise ValueError(f"stimuli_features shape expected (T,26), got {sf.shape}")
            stimuli_features = np.asarray(sf, dtype=np.float32)
            # Trim/pad to T
            if stimuli_features.shape[0] != T:
                L = min(T, stimuli_features.shape[0])
                stimuli_features = np.pad(stimuli_features[:L], ((0, T - L), (0, 0)), mode='edge') if L < T else stimuli_features[:T]
            ...
            if candidate_ids:
                ...
                print(f"  Stimuli features loaded: shape={vals.shape}; derived K={K} identity labels from {len(use)} identity dims")
            ...
            print(f"  → Warning: failed to load stimuli_features ({e}); falling back to stimuli_store if available")
        ...
        try:
            ...
            if st is not None:
                ...
                if stimuli_labels.shape[0] != T:
                    ...
                    if L < T:
                        pad = np.full((T - L,), stimuli_labels[-1] if L > 0 else 0, dtype=np.int64)
                ...
                compact = np.vectorize(lambda x: label_to_compact[int(x)])(stimuli_labels)
                K = int(len(uniq))
                stimuli_onehot = np.eye(K, dtype=np.float32)[compact]
                print(f"  Stimuli loaded: K={K} classes, length={stimuli_labels.shape[0]}")
            else:
                ...
                item = 4
                Tstim = size // (ch * item)
                mm = np.memmap(str(stim_store), dtype=np.float32, mode='r', shape=(Tstim, ch))
                ...
                for t_idx in range(T):
                    s = int(edges[t_idx]); e = int(edges[t_idx+1])
                    if e <= s:
                        e = min(Tstim, s + 1)
                    mm_slice = mm[s:e]
                ...
                K = int(len(uniq))
                stimuli_onehot = np.eye(K, dtype=np.float32)[compact]
                print(f"  Stimuli derived from 10ch float: K={K} classes, length={stimuli_labels.shape[0]}")
        except Exception as e:
            print(f"  → Warning: failed to load stimuli ({e}); continuing without stimuli")
    ...
    for z in range(Z):
        ...
        sz = np.bincount(labels, weights=np.full(labels.shape[0], float(z)), minlength=N + 1)
        count[:cnt.size] += cnt
        sum_x[:sx.size] += sx
        sum_y[:sy.size] += sy
        sum_z[:sz.size] += sz
    ...
    pos = np.stack([cx[keep_mask], cy[keep_mask], cz[keep_mask]], axis=1).astype(np.float32)
    mins = np.array([0.0, 0.0, 0.0], dtype=np.float32)
    maxs = np.array([max(1.0, X - 1), max(1.0, Y - 1), max(1.0, Z - 1)], dtype=np.float32)
    ...
    return_to_original = bool(config['processing'].get('return_to_original_rate', False))
    ...
    if skip_cascade:
        ...
        with h5py.File(final_file, 'w') as f:
            ...
            f.create_dataset('zscore_std', data=std32.astype(output_dtype), compression='gzip', compression_opts=1)
        ...
        try:
            ...
            stim=(stimuli_labels if stimuli_labels is not None else None), behavior=None, eye=None,
                is_probability=False, value_label='Z-scored ZapBench Signal',
            )
        except Exception as e:
    ...
    if not CASCADE_AVAILABLE:
        raise RuntimeError("CASCADE not available. Set processing.skip_cascade: true or install neuralib.")
    if not orig_rate or not tgt_rate:
        raise RuntimeError("original_sampling_rate and target_sampling_rate are required for CASCADE path")
    ...
    if return_to_original and float(orig_rate) != float(tgt_rate):
        ...
        from scipy.signal import resample_poly
        from fractions import Fraction
        rate_ratio = Fraction(int(orig_rate)).limit_denominator() / Fraction(int(tgt_rate)).limit_denominator()
        up_factor = rate_ratio.numerator
        down_factor = rate_ratio.denominator
        ...
        for n in tqdm(range(N_keep), desc="Downsampling neurons"):
            downsampled[:, n] = resample_poly(prob_data_T[:, n], up_factor, down_factor, axis=0)
    ...
    with h5py.File(final_file, 'w') as f:
        dset_vals = f.create_dataset(
            'neuron_values', data=prob_data.T.astype(output_dtype),
            chunks=(min(1000, T), min(256, N_keep)), compression='gzip', compression_opts=1,
        )
        f.create_dataset('cell_positions', data=pos_norm.astype(output_dtype), compression='gzip', compression_opts=1)
        f.create_dataset('timepoint_indices', data=np.arange(T, dtype=np.int32))
        f.create_dataset('num_timepoints', data=T)
...
def _split_and_save_by_stimuli(h5_path: str, output_dir: str):
    ...
    with h5py.File(str(path), 'r') as f:
        ...
        if 'stimulus_full' in f:
            ...
            try:
                if 'stimulus_condition_names' in f.attrs:
                    cond_names = [s.decode('utf-8') if isinstance(s, (bytes, bytearray)) else str(s) for s in f.attrs['stimulus_condition_names']]
            except Exception:
                cond_names = None
            ...
            if cand.size == 0:
                print("  → No identity-like dims in stimuli_features; skipping split-by-stimuli")
                return
            active = feats[:, cand]
        ...
        for i in range(len(starts)-1):
            ...
            label = int(labels[s])
            label_name = None
            if 'cond_names' in locals() and cond_names is not None and label < len(cond_names):
            ...
            with h5py.File(out_h5, 'w') as g:
                g.create_dataset('neuron_values', data=values[s:e].astype(values.dtype), compression='gzip', compression_opts=1)
                g.create_dataset('cell_positions', data=positions, compression='gzip', compression_opts=1)
                g.create_dataset('timepoint_indices', data=np.arange(e - s, dtype=np.int32))
                ...
                if 'stim_onehot' in locals():
                    g.create_dataset('stimulus_full', data=stim_onehot[s:e], compression='gzip', compression_opts=1)
                # Save full 26-D features if present in source file
                if 'feats' in locals():
                    g.create_dataset('stimuli_features', data=feats[s:e].astype(np.float32), compression='gzip', compression_opts=1)
                g.attrs.update({k: f.attrs[k] for k in f.attrs.keys() if k not in ('num_timepoints',)})
                # Add train/val/test split ranges per paper (TAXIS fully test)
                L = int(e - s)
                g.attrs['split_index_semantics'] = '0-based inclusive start, exclusive end'
                g.attrs['split_condition_name'] = (label_name if label_name else f'stim{label}')
            ...
            try:
                ...
                create_visualization_pdf(values[s:e], values[s:e].T, f.attrs.get('subject','zapbench'), out_pdf,
                                         num_neurons=min(10, N), stim=stim_for_pdf, behavior=None, eye=None,
                                         is_probability=False, value_label='Segment Signal')
