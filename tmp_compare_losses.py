...
from GenerativeBrainModel.scripts.eval_gbm import (
    create_default_config as eval_default_config,
)
...
def compute_mean_loss(
    model: GBM, loader: torch.utils.data.DataLoader, device: torch.device, limit: int
) -> float:
    losses = []
    with torch.no_grad():
...
def main() -> None:
    ...
    eval_cfg_path = Path(
        "/home/user/gbm3/GBM3/experiments/gbm_neural_eval/gbm_neural_evaluation_20250926_103035/config.yaml"
    )
    ckpt_path = Path(
