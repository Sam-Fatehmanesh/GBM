    ...
    try:
        ...
        with h5py.File(filepath, "r") as f:
            ...
            if total_entries > 50_000_000:  # 50M elements
                sample_size = min(
                    1_000_000, total_entries // 100
                )  # Sample 1% or 1M, whichever is smaller
                ...
                time_indices = np.random.choice(
                    shape[0], n_timepoints_sample, replace=False
                )
                ...
                for t_idx in time_indices:
                    ...
                    if volume_slice.size > sample_size // n_timepoints_sample:
                        ...
                        spatial_indices = np.random.choice(
                            flat_volume.size, spatial_sample_size, replace=False
                        )
                ...
                if GPU_AVAILABLE:
                    ...
                    if non_zero_sample > 0:
                        ...
                        min_val = max_val = mean_val = mean_nonzero_val = std_val = (
                            median_nonzero_val
                        ) = 0
                else:
            ...
            if verbose:
                ...
                print(
                    f"  {'Estimated' if is_sampled else 'Actual'} non-zero entries: {estimated_non_zero:,}"
                )
                print(
                    f"  Fraction non-zero: {fraction_nonzero:.6f} ({fraction_nonzero * 100:.4f}%)"
                )
                ...
                if is_sampled:
                    print(
                        f"  NOTE: Statistics based on random sample due to large dataset size"
                    )
            ...
            return {
                ...
                "mean_nonzero_value": float(mean_nonzero_val)
                if estimated_non_zero > 0
                else None,
                "median_nonzero_value": float(median_nonzero_val)
                if estimated_non_zero > 0
                else None,
...
def print_summary_table(results):
    ...
    for result in results:
        ...
        avg_prob = (
            f"{result['mean_value']:.4f}" if result["mean_value"] is not None else "N/A"
        )
...
def print_detailed_stats(results):
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            f"   Timepoints: {result['num_timepoints']:,} | Volume: {result['volume_shape'][0]}×{result['volume_shape'][1]}×{result['volume_shape'][2]}"
        )
        print(f"   Total entries: {result['total_entries']:,}")
        print(
            f"   Non-zero entries: {result['estimated_non_zero_entries']:,} ({'estimated' if result['is_sampled'] else 'exact'})"
        )
        print(
            f"   Sparsity: {result['sparsity'] * 100:.4f}% | Non-zero: {result['fraction_nonzero'] * 100:.4f}%"
        )

        if result["mean_nonzero_value"] is not None:
            print(
                f"   Values - Min: {result['min_nonzero_value']:.6f} | Max: {result['max_value']:.6f}"
            )
            print(
                f"   Mean (all): {result['mean_value']:.6f} | Mean (non-zero): {result['mean_nonzero_value']:.6f}"
            )
            print(
                f"   Median (non-zero): {result['median_nonzero_value']:.6f} | Std dev: {result['std_deviation']:.6f}"
            )
        else:
            print(f"   All values are zero")

        print(
            f"   File size: {result['file_size_human']} | Memory: {result['estimated_memory_gb']:.2f} GB"
        )
        print(
            f"   Memory per timepoint: {format_bytes(result['memory_per_timepoint_bytes'])}"
        )
...
def print_aggregate_stats(results):
    ...
    weighted_mean = (
        sum(r["mean_value"] * r["total_entries"] for r in results) / total_entries
    )
    ...
    print(
        f"Overall non-zero fraction: {overall_nonzero_fraction:.6f} ({overall_nonzero_fraction * 100:.4f}%)"
    )
    ...
    if all_sparsities:
        print(
            f"Sparsity range - Min: {min(all_sparsities) * 100:.2f}% | Max: {max(all_sparsities) * 100:.2f}%"
        )

    if all_means:
        print(
            f"Mean spike prob range - Min: {min(all_means):.6f} | Max: {max(all_means):.6f}"
        )
    ...
    output_data = {
        ...
        "summary": {
            ...
            "total_nonzero_entries": sum(
                r["estimated_non_zero_entries"] for r in results
            ),
            "overall_sparsity": (
                sum(r["total_entries"] for r in results)
                - sum(r["estimated_non_zero_entries"] for r in results)
            )
            / sum(r["total_entries"] for r in results),
...
Examples:
  ...
  python analyze_processed_spikes.py /path/to/data --verbose --save-json results.json
    ...
    parser.add_argument(
        "directory", help="Directory containing processed spike H5 files"
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Verbose output for each file"
    )
    ...
    for h5_file in sorted(h5_files):
        result = analyze_h5_file(
            h5_file, verbose=args.verbose and not args.summary_only
        )
        if result:
