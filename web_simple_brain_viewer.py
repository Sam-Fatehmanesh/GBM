...
def load_neuron_positions(h5_path, max_neurons=100000):
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        if num_neurons > max_neurons:
            print(f"  Subsampling {num_neurons} neurons to {max_neurons} for visualization")
        ...
        print(f"  Position ranges (normalized): X=[{cell_positions[:, 0].min():.3f}, {cell_positions[:, 0].max():.3f}], "
        ...
        print(f"  Position ranges (scaled): X=[{cell_positions_scaled[:, 0].min():.1f}, {cell_positions_scaled[:, 0].max():.1f}], "
...
def load_mask_data(h5_path):
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        region_names = [name.decode() if isinstance(name, bytes) else str(name) 
...
<html>
<head>
    <title>Simple Brain Viewer - v""" + str(timestamp) + """</title>
    <meta charset="utf-8">
...
def api_region_surface(region_id):
    ...
    try:
        ...
        if region_id < 1 or region_id > MASK_DATA['num_regions']:
            return jsonify({'error': f'Invalid region ID {region_id}, valid range: 1-{MASK_DATA["num_regions"]}'}), 400
        
        # Extract surface for the region
        vertices = extract_region_surfaces(MASK_DATA['label_volume'], region_id, subsample=4)
        return jsonify({
            'region_id': region_id,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simple web viewer for brain neuron positions and masks')
    parser.add_argument('--subject-h5', required=True, help='Path to subject HDF5 file with neuron positions')
    parser.add_argument('--mask-h5', required=True, help='Path to mask HDF5 file with region data')
    ...
    # Load data
    print("Loading data...")
    NEURON_DATA = load_neuron_positions(args.subject_h5, max_neurons=100000)  # Show all neurons - no subsampling
